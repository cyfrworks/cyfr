...
/// (`ids::valid`, with nothing beyond its base characters: a `:` would name
/// a method in the URL). A model that does not is refused before any
/// request.
...
pub fn describe_model(api_key: &str, model: &str) -> String {
    let req = json!({
        "method": "GET",
        "url": format!("{BASE_URL}/models/{}", ids::segment(model)),
    ...
    let envelope = match outcome {
        Outcome::Completed => match assembly.error {
            ...
            None if !assembly.complete() => refuse(
                502,
                "incomplete_stream",
                "the stream ended before the provider finished its candidate",
            ),
            None => {
...
/// and is a call of its own. The stream is whole once a candidate carries
/// its `finishReason`, or the prompt is blocked.
#[derive(Default)]
pub struct Assembly {
    parts: Vec<Value>,
    calls: u64,
...
impl Assembly {
    ...
    /// Whether the provider finished its candidate or blocked the prompt.
    fn complete(&self) -> bool {
        let blocked = self
            .prompt_feedback
            .as_ref()
            .is_some_and(|feedback| feedback.get("blockReason").is_some());
        self.finish_reason.is_some() || blocked
    }

    /// The whole response, as generateContent answers it without a stream:
    /// a blocked prompt that no candidate answered carries none.
    pub fn response(&self) -> Value {
        let mut data = json!({});
        if !self.parts.is_empty() || self.finish_reason.is_some() {
            let mut candidate = json!({"content": {"role": "model", "parts": self.parts}});
            if let Some(reason) = &self.finish_reason {
                candidate["finishReason"] = reason.clone();
            }
            data["candidates"] = json!([candidate]);
        }
        for (key, slot) in [
...
mod tests {
    ...
    #[test]
    fn a_stream_that_ends_before_its_finish_reason_is_refused_as_incomplete() {
        let transcript = [
            chunk(json!({"candidates": [{"content": {"role": "model", "parts": [{"text": "Let me "}]}}]})),
            chunk(json!({
                ...
                "usageMetadata": {"promptTokenCount": 130, "candidatesTokenCount": 5}
            })),
        ]
        .concat();
        ...
        let blocked = chunk(json!({"promptFeedback": {"blockReason": "SAFETY"},
            "usageMetadata": {"promptTokenCount": 7}}));
        let (envelope, _) = streamed(&blocked);
        assert_eq!(envelope["status"], 200);
        assert_eq!(envelope["data"]["stop_reason"], "content_filter");
    }

    #[test]
    fn two_calls_to_one_function_stay_two_calls() {
        let transcript = [
            chunk(json!({"candidates": [{"content": {"role": "model", "parts": [
                {"functionCall": {"name": "files.read", "args": {"path": "a"}}}
            ]}}]})),
            chunk(json!({"candidates": [{"finishReason": "STOP", "content": {"role": "model", "parts": [
                {"functionCall": {"name": "files.read", "args": {"path": "b"}}}
            ]}}]})),
        ]
        .concat();
        ...
        assert_eq!(
            ...
            json!([
                {"type": "tool_call", "id": "call_1", "name": "files.read", "arguments": {"path": "a"}},
                {"type": "tool_call", "id": "call_2", "name": "files.read", "arguments": {"path": "b"}}
            ])
        );
        ...
        assert_eq!(
            calls,
            vec![
                &json!({"type": "tool_call.start", "index": 0, "id": "call_1", "name": "files.read"}),
                &json!({"type": "tool_call.delta", "index": 0, "arguments": "{\"path\":\"a\"}"}),
                &json!({"type": "tool_call.end", "index": 0}),
                &json!({"type": "tool_call.start", "index": 1, "id": "call_2", "name": "files.read"}),
                &json!({"type": "tool_call.delta", "index": 1, "arguments": "{\"path\":\"b\"}"}),
