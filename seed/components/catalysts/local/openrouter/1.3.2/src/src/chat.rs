...
/// (`ids::valid`, with the `/`, `:` and `~` its ids use). A model that does
/// not is refused before any request.
pub fn model_id(model: &str) -> bool {
    ids::valid(model, &['/', ':', '~'])
}

/// The refusal of a model this provider does not know.
...
fn answer<S: Sink>(
    ...
    let envelope = match outcome {
        Outcome::Completed => match assembly.error.take() {
            ...
            None if !assembly.complete() => refuse(
                502,
                "incomplete_stream",
                "the stream ended before the provider finished its choice and closed the stream",
            ),
            None => {
...
/// One streamed tool call: its id and name from the fragment that started
/// it, its arguments joined across the fragments that continue it.
struct Call {
    id: String,
    name: String,
    arguments: String,
...
/// the contract's stream events as it goes. Tool calls are kept in the
/// order they start and numbered so; the stream's own index only says
/// which open call a fragment continues. A fragment with an id continues
/// the open call that has it, or else starts a call of its own, ending the
/// call open at its index; a fragment without an id continues the call open
/// at its index. Every call still open ends when the choice finishes. The
/// stream is whole once the choice has its `finish_reason` and `[DONE]`
/// closes the stream.
#[derive(Default)]
pub struct Assembly {
    model: Option<Value>,
    text: String,
    calls: Vec<Call>,
    // A stream index to the position of the call open at it.
...
impl Assembly {
    ...
    fn call_fragment<S: Sink>(&mut self, fragment: &Value, deltas: &mut Deltas<S>) {
        ...
        let id = fragment["id"].as_str().filter(|id| !id.is_empty());

        let continued = match id {
            Some(id) => self
                .open
                .values()
                .copied()
                .find(|position| self.calls[*position].id == id),
            None => self.open.get(&index).copied(),
        };
        let position = match continued {
            Some(position) => position,
            None => {
                if let Some(replaced) = self.open.remove(&index) {
                    deltas.event(stream::tool_call_end(replaced as u64));
                }
                let call = Call {
                ...
                deltas.event(stream::tool_call_start(position as u64, &call.id, &call.name));
                self.calls.push(call);
                self.open.insert(index, position);
                position
            }
        };
        ...
        if let Some(arguments) = function.get("arguments").and_then(Value::as_str) {
            ...
            if !arguments.is_empty() {
                deltas.arguments(position as u64, arguments);
            }
        }
    }

    /// End every call still open, in the order they started.
    fn end_calls<S: Sink>(&mut self, deltas: &mut Deltas<S>) {
        let mut open: Vec<usize> = std::mem::take(&mut self.open).into_values().collect();
        open.sort_unstable();
        for position in open {
            deltas.event(stream::tool_call_end(position as u64));
        }
    }

    /// Whether the choice finished and the stream was closed.
    fn complete(&self) -> bool {
        self.finish_reason.is_some() && self.done
    }
...
mod tests {
    ...
    fn a_streamed_answer_emits_its_events_and_assembles_the_same_response() {
        ...
        assert_eq!(
            kinds,
            vec![
                "text.delta", "tool_call.start", "tool_call.delta", "tool_call.start",
                "tool_call.delta", "tool_call.end", "tool_call.end", "usage", "stop"
            ]
        );
        assert_eq!(events[0]["text"], "Looking.");
        assert_eq!(events[1]["id"], "call_9");
        assert_eq!(events[2]["arguments"], "{\"path\":\"b\"}");
        assert_eq!(events[3], json!({"type": "tool_call.start", "index": 1, "id": "call_10", "name": "files.list"}));
        assert_eq!(events[5], json!({"type": "tool_call.end", "index": 0}));
        assert_eq!(events[6], json!({"type": "tool_call.end", "index": 1}));
    ...
    #[test]
    fn a_stream_without_its_finish_reason_or_done_is_refused_as_incomplete() {
        let text = chunk(json!({"index": 0, "delta": {"content": "Look"}, "finish_reason": null}));
        let finished = chunk(json!({"index": 0, "delta": {}, "finish_reason": "stop"}));
        let usage = format!("data: {}\n\n", json!({"id": "gen-1", "choices": [],
            "usage": {"prompt_tokens": 4, "completion_tokens": 1}}));
        let done = "data: [DONE]\n\n".to_string();

        let whole = [text.clone(), finished.clone(), usage.clone(), done.clone()].concat();
        let (envelope, _) = streamed(&whole);
        assert_eq!(envelope["status"], 200);
        assert_eq!(envelope["data"]["content"][0]["text"], "Look");
        assert_eq!(envelope["data"]["usage"]["output_tokens"], 1);

        let cut_mid_text = text.clone();
        let cut_before_done = [text.clone(), finished, usage.clone()].concat();
        let done_without_finish = [text, usage, done].concat();

        for transcript in [cut_mid_text, cut_before_done, done_without_finish] {
    ...
    #[test]
    fn distinct_tool_calls_sharing_an_index_stay_distinct() {
        let fragment = |call: Value| {
            chunk(json!({"index": 0, "delta": {"tool_calls": [call]}, "finish_reason": null}))
        };
        let transcript = [
            fragment(json!({"index": 0, "id": "call_a", "type": "function",
                "function": {"name": "files.read", "arguments": "{\"pa"}})),
            fragment(json!({"index": 0, "function": {"arguments": "th\":\"a\"}"}})),
            fragment(json!({"index": 0, "id": "call_b", "type": "function",
                "function": {"name": "files.list", "arguments": ""}})),
            fragment(json!({"index": 0, "id": "call_b", "function": {"arguments": "{\"dir\":"}})),
            fragment(json!({"index": 0, "function": {"arguments": "\"b\"}"}})),
            chunk(json!({"index": 0, "delta": {}, "finish_reason": "tool_calls"})),
            "data: [DONE]\n\n".to_string(),
        ]
        .concat();
        ...
        assert_eq!(
            ...
            json!([
                {"type": "tool_call", "id": "call_a", "name": "files.read", "arguments": {"path": "a"}},
                {"type": "tool_call", "id": "call_b", "name": "files.list", "arguments": {"dir": "b"}}
            ])
        );
    ...
    fn a_refused_or_broken_stream_answers_a_typed_refusal_and_emits_it() {
        ...
        .unwrap();
        assert_eq!(empty["error"]["type"], "incomplete_stream");
