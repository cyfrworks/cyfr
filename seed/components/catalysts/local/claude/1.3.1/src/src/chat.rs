...
pub fn describe_model(api_key: &str, model: &str) -> String {
    let req = json!({
        "method": "GET",
        "url": format!("{BASE_URL}/v1/models/{}", ids::segment(model)),
    ...
    let envelope = match outcome {
        Outcome::Completed => match assembly.error {
            ...
            None if !assembly.complete() => refuse(
                502,
                "incomplete_stream",
                "the stream ended before the provider closed its message",
            ),
            None => {
...
/// One content block as it streams: the block its start carried, the JSON
/// input joined across its deltas, and the tool call it is, if one.
struct Streamed {
    block: Value,
    input: String,
    call: Option<u64>,
...
/// contract's stream events as it goes. Blocks are kept in the order they
/// start; a delta or stop names the block open at its index, and a start at
/// an index whose block is still open closes that block and starts another.
/// The stream is whole once `message_stop` arrives with no block open.
#[derive(Default)]
pub struct Assembly {
    model: Option<String>,
    blocks: Vec<Streamed>,
    // A stream index to the position of the block open at it.
...
impl Assembly {
    pub fn apply<S: Sink>(&mut self, frame: &Frame, deltas: &mut Deltas<S>) {
        ...
        match data.get("type").and_then(Value::as_str) {
            ...
            Some("content_block_start") => {
                if let Some(position) = self.open.remove(&index) {
                    self.close(position, deltas);
                }
                let block = data["content_block"].clone();
                let call = (block["type"] == "tool_use").then(|| {
                    let call = self.calls;
                    self.calls += 1;
                ...
                self.open.insert(index, self.blocks.len());
                self.blocks.push(Streamed {
                    block,
                    input: String::new(),
                    call,
                });
            ...
            Some("content_block_delta") => {
                let Some(streamed) = self.open.get(&index).map(|p| &mut self.blocks[*p]) else {
                    return;
                };
                ...
                match delta["type"].as_str() {
                    Some("text_delta") => {
                        ...
                        let joined = format!("{}{text}", streamed.block["text"].as_str().unwrap_or(""));
                        streamed.block["text"] = json!(joined);
                        deltas.text(text);
                    }
                    Some("input_json_delta") => {
                        let fragment = delta["partial_json"].as_str().unwrap_or("");
                        streamed.input.push_str(fragment);
                        if let Some(call) = streamed.call {
    ...
    /// End the block at `position`: a tool call takes its joined input as
    /// its arguments and emits its end.
    fn close<S: Sink>(&mut self, position: usize, deltas: &mut Deltas<S>) {
        let streamed = &mut self.blocks[position];
        if let Some(call) = streamed.call {
            let input = streamed.input.trim();
            streamed.block["input"] = if input.is_empty() {
                json!({})
            } else {
    ...
    /// Whether the provider closed the message and every block in it.
    fn complete(&self) -> bool {
        self.stopped && self.open.is_empty()
    }
    ...
    pub fn response(&self) -> Value {
        json!({
            "model": self.model,
            "content": self.blocks.iter().map(|s| s.block.clone()).collect::<Vec<_>>(),
...
mod tests {
    ...
    #[test]
    fn a_stream_that_ends_before_message_stop_is_refused_as_incomplete() {
        let cut_before_stop = STREAM.strip_suffix("event: message_stop\ndata: {\"type\":\"message_stop\"}\n\n").unwrap();
        let cut_mid_text = STREAM.split("event: ping").next().unwrap();
        let open_block = STREAM.replace(
            "event: content_block_stop\ndata: {\"type\":\"content_block_stop\",\"index\":1}\n\n",
            "",
        );

        for transcript in [cut_before_stop, cut_mid_text, open_block.as_str()] {
    ...
    #[test]
    fn two_tool_calls_started_at_one_index_stay_two_calls() {
        let block = |kind: &str, body: Value| {
            let mut data = body;
            data["type"] = json!(kind);
            format!("event: {kind}\ndata: {data}\n\n")
        };
        let start = |id: &str, name: &str| {
            block("content_block_start", json!({"index": 0,
                "content_block": {"type": "tool_use", "id": id, "name": name, "input": {}}}))
        };
        let input = |json: &str| {
            block("content_block_delta", json!({"index": 0,
                "delta": {"type": "input_json_delta", "partial_json": json}}))
        };
        let transcript = [
            block("message_start", json!({"message": {"model": "claude-x", "usage": {}}})),
            start("toolu_a", "files.read"),
            input("{\"path\":\"a\"}"),
            start("toolu_b", "files.list"),
            input("{\"dir\":"),
            input("\"b\"}"),
            block("content_block_stop", json!({"index": 0})),
            block("message_delta", json!({"delta": {"stop_reason": "tool_use"}, "usage": {}})),
            block("message_stop", json!({})),
        ]
        .concat();

        let (envelope, events) = streamed(&transcript);
        let content = envelope["data"]["content"].as_array().unwrap();
        assert_eq!(
            content,
            &vec![
                json!({"type": "tool_call", "id": "toolu_a", "name": "files.read", "arguments": {"path": "a"}}),
                json!({"type": "tool_call", "id": "toolu_b", "name": "files.list", "arguments": {"dir": "b"}}),
            ]
        );
        ...
        assert_eq!(
            calls,
            vec![
                &json!({"type": "tool_call.start", "index": 0, "id": "toolu_a", "name": "files.read"}),
                &json!({"type": "tool_call.delta", "index": 0, "arguments": "{\"path\":\"a\"}"}),
                &json!({"type": "tool_call.end", "index": 0}),
                &json!({"type": "tool_call.start", "index": 1, "id": "toolu_b", "name": "files.list"}),
