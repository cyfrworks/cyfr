...
// Each model's window, as xAI's model pages state it, by every id xAI
// serves the model under: its release id, its aliases, and the retired ids
// it answers. An id is matched whole; any other is not a model this
// catalyst knows. xAI documents no output ceiling for these models, and the
// Models API reports neither limit.
const MODELS: &[(&[&str], u64)] = &[
    (&["grok-4.6"], 500_000),
    (&["grok-4.5", "grok-4.5-latest", "grok-build-latest"], 500_000),
    (
        &[
            "grok-4.3",
            "grok-4.3-latest",
...
/// (`ids::valid`, with nothing beyond its base characters). A model that
/// does not is refused before any request.
...
/// The window of the one row that names `model`.
fn window(model: &str) -> Option<u64> {
    MODELS
        .iter()
        .find(|(ids, _)| ids.contains(&model))
...
mod tests {
    ...
    fn describe_names_the_contract_and_a_known_models_window() {
        ...
        assert_eq!(current["data"]["model"], "grok-4.20-0309-reasoning");
    ...
    fn window_of(model: &str) -> Result<Value, Value> {
        let described: Value = serde_json::from_str(&describe(&json!({"model": model}))).unwrap();
        match described["status"].as_i64() {
            Some(200) => Ok(described["data"]["context_window"].clone()),
            _ => Err(described["error"]["type"].clone()),
        }
    }

    #[test]
    fn a_model_is_known_by_an_id_xai_serves_it_under_and_nothing_else() {
        for (model, context) in [
            ...
            assert_eq!(window_of(model), Ok(json!(context)), "{model}");
        }
        ...
        for invented in [
            ...
            assert_eq!(window_of(invented), Err(json!("unknown_model")), "{invented}");
        }
    }

    #[test]
    fn every_id_a_row_names_keeps_to_the_id_grammar_and_no_other_row_names_it() {
        for (named, _) in MODELS {
            for id in *named {
                assert!(model_id(id), "{id}");
                let rows = MODELS.iter().filter(|(other, _)| other.contains(id)).count();
                assert_eq!(rows, 1, "{id}");
            }
        }
    }
