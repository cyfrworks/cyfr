...
/// Which ids a model row names.
#[derive(Clone, Copy)]
enum Ids {
    /// Its id alone.
    Exact,
    /// Its id, and its dated snapshots: `<id>-YYYY-MM-DD`.
    Dated,
}

// Each model's window and output ceiling, as OpenAI's model pages state
// them, by the ids that name it. Any other id is not a model this catalyst
// knows. The Models API reports neither limit.
const MODELS: &[(&str, Ids, u64, u64)] = &[
    ("gpt-6-astra", Ids::Exact, 1_050_000, 128_000),
    ("gpt-5.6", Ids::Exact, 1_050_000, 128_000),
    ("gpt-5.6-sol", Ids::Exact, 1_050_000, 128_000),
    ("gpt-5.6-terra", Ids::Exact, 1_050_000, 128_000),
    ("gpt-5.6-luna", Ids::Exact, 1_050_000, 128_000),
    ("gpt-daybreak-blue-latest", Ids::Exact, 1_050_000, 128_000),
    ("gpt-5.6-cyber", Ids::Exact, 400_000, 128_000),
    ("gpt-daybreak-red-latest", Ids::Exact, 400_000, 128_000),
    ("gpt-5.5", Ids::Dated, 1_050_000, 128_000),
    ("gpt-5.5-pro", Ids::Dated, 1_050_000, 128_000),
    ("gpt-5.4", Ids::Dated, 1_050_000, 128_000),
    ("gpt-5.4-pro", Ids::Dated, 1_050_000, 128_000),
    ("gpt-5.4-mini", Ids::Dated, 400_000, 128_000),
    ("gpt-5.4-nano", Ids::Dated, 400_000, 128_000),
    ("gpt-5.3-codex", Ids::Exact, 400_000, 128_000),
    ("gpt-5.2", Ids::Dated, 400_000, 128_000),
    ("gpt-5.2-pro", Ids::Dated, 400_000, 128_000),
    ("gpt-5.1", Ids::Dated, 400_000, 128_000),
    ("gpt-5", Ids::Dated, 400_000, 128_000),
    ("gpt-5-mini", Ids::Dated, 400_000, 128_000),
    ("gpt-5-nano", Ids::Dated, 400_000, 128_000),
    ("gpt-5-pro", Ids::Dated, 400_000, 272_000),
    ("chat-latest", Ids::Exact, 400_000, 128_000),
    ("gpt-4.1", Ids::Dated, 1_047_576, 32_768),
    ("gpt-4.1-mini", Ids::Dated, 1_047_576, 32_768),
    ("gpt-4.1-nano", Ids::Dated, 1_047_576, 32_768),
    // The gpt-4o snapshots do not share one output ceiling.
    ("gpt-4o", Ids::Exact, 128_000, 16_384),
    ("gpt-4o-2024-11-20", Ids::Exact, 128_000, 16_384),
    ("gpt-4o-2024-08-06", Ids::Exact, 128_000, 16_384),
    ("gpt-4o-2024-05-13", Ids::Exact, 128_000, 4_096),
    ("gpt-4o-mini", Ids::Dated, 128_000, 16_384),
    ("o4-mini", Ids::Dated, 200_000, 100_000),
    ("o3", Ids::Dated, 200_000, 100_000),
    ("o3-pro", Ids::Dated, 200_000, 100_000),
    ("o3-mini", Ids::Dated, 200_000, 100_000),
    ("o1", Ids::Dated, 200_000, 100_000),
    ("o1-pro", Ids::Dated, 200_000, 100_000),
];
...
/// Whether `model` keeps to the grammar of this provider's model ids
/// (`ids::valid`, with the `:` its fine-tuned ids use). A model that does
/// not is refused before any request.
pub fn model_id(model: &str) -> bool {
    ids::valid(model, &[':'])
}

/// The refusal of a model this catalyst does not know.
...
/// The window and output ceiling of the one row that names `model`.
fn window(model: &str) -> Option<(u64, u64)> {
    MODELS
        .iter()
        .find(|(id, ids, _, _)| names(id, *ids, model))
        .map(|(_, _, context, output)| (*context, *output))
}

/// Whether `model` is `id` whole or, for a dated row, one of its snapshots.
fn names(id: &str, ids: Ids, model: &str) -> bool {
    match ids {
        Ids::Exact => model == id,
        Ids::Dated => {
            model == id
    ...
    let envelope = match outcome {
        Outcome::Completed => match (assembly.error, assembly.response) {
            ...
            (None, None) => refuse(
                502,
                "incomplete_stream",
                "the stream ended before the provider sent its final response",
            ),
        },
...
/// A function call whose output item is still streaming.
struct OpenCall {
    index: u64,
    item: Option<String>,
    output: u64,
...
/// emitting the contract's stream events as it goes. Each function call
/// output item is a call of its own, numbered in the order they start. An
/// event names its call by output item id where both carry one, and by
/// output index otherwise; an item added at the output index of a call
/// still open ends that call. Every call still open ends with the final
/// response, which is the answer.
#[derive(Default)]
pub struct Assembly {
    open: Vec<OpenCall>,
    calls: u64,
...
impl Assembly {
    pub fn apply<S: Sink>(&mut self, frame: &Frame, deltas: &mut Deltas<S>) {
        ...
        match data.get("type").and_then(Value::as_str) {
            Some("response.output_item.added") if data["item"]["type"] == "function_call" => {
                if let Some(replaced) = self.open.iter().position(|c| c.output == output) {
                    self.end(replaced, deltas);
                }
                ...
                self.open.push(OpenCall {
                    index,
                    item: item_id(&data).map(str::to_string),
                    output,
                });
            }
            ...
            Some("response.function_call_arguments.delta") => {
                if let Some(call) = self.open_call(&data, output) {
                    deltas.arguments(self.open[call].index, data["delta"].as_str().unwrap_or(""));
                }
            }
            Some("response.output_item.done") => {
                if let Some(call) = self.open_call(&data, output) {
                    self.end(call, deltas);
                }
            }
    ...
    /// The open call an event names: by output item id where the event and
    /// the call both carry one, by output index otherwise.
    fn open_call(&self, data: &Value, output: u64) -> Option<usize> {
        let item = item_id(data);
        self.open
            .iter()
            .position(|call| match (item, call.item.as_deref()) {
                (Some(item), Some(open)) => item == open,
                _ => call.output == output,
            })
    }

    fn end<S: Sink>(&mut self, call: usize, deltas: &mut Deltas<S>) {
        let call = self.open.remove(call);
        deltas.event(stream::tool_call_end(call.index));
    }
}

/// The output item id an event carries: its own `item_id`, or its item's.
fn item_id(data: &Value) -> Option<&str> {
    data.get("item_id")
...
mod tests {
    ...
    fn describe_names_the_contract_and_a_known_models_window() {
        ...
        let known: Value =
            serde_json::from_str(&describe(&json!({"model": "gpt-5-mini"}))).unwrap();
        assert_eq!(known["data"]["context_window"], 400000);
        assert_eq!(known["data"]["max_output_tokens"], 128000);

        let sol: Value =
            serde_json::from_str(&describe(&json!({"model": "gpt-5.6-sol"}))).unwrap();
        assert_eq!(sol["data"]["context_window"], 1050000);
    ...
    fn limits(model: &str) -> Result<(Value, Value), Value> {
        let described: Value = serde_json::from_str(&describe(&json!({"model": model}))).unwrap();
        match described["status"].as_i64() {
            Some(200) => Ok((
                described["data"]["context_window"].clone(),
                described["data"]["max_output_tokens"].clone(),
            )),
            _ => Err(described["error"]["type"].clone()),
        }
    }

    #[test]
    fn a_model_is_known_by_its_id_or_a_dated_snapshot_and_nothing_else() {
        for (model, context, output) in [
            ("gpt-5", 400_000, 128_000),
            ("gpt-5-2025-08-07", 400_000, 128_000),
            ("gpt-5-mini-2025-08-07", 400_000, 128_000),
            ("gpt-5-pro", 400_000, 272_000),
            ("gpt-5.4", 1_050_000, 128_000),
            ("gpt-5.4-mini-2026-03-17", 400_000, 128_000),
            ("gpt-6-astra", 1_050_000, 128_000),
            ("gpt-4.1-2025-04-14", 1_047_576, 32_768),
            ("gpt-4o-2024-05-13", 128_000, 4_096),
            ("o4-mini-2025-04-16", 200_000, 100_000),
        ] {
            assert_eq!(limits(model), Ok((json!(context), json!(output))), "{model}");
        }
        ...
        for invented in [
            ...
            assert_eq!(limits(invented), Err(json!("unknown_model")), "{invented}");
        }
    }

    #[test]
    fn every_id_a_row_names_keeps_to_the_id_grammar_and_no_other_row_names_it() {
        for (id, forms_of, _, _) in MODELS {
            let mut forms = vec![id.to_string()];
            if let Ids::Dated = forms_of {
                forms.push(format!("{id}-2026-01-01"));
            }
            for form in forms {
                assert!(model_id(&form), "{form}");
                let rows = MODELS.iter().filter(|(other, ids, _, _)| names(other, *ids, &form)).count();
                assert_eq!(rows, 1, "{form}");
            }
        }
    }
    ...
    fn a_refused_or_broken_stream_answers_a_typed_refusal_and_emits_it() {
        ...
        .unwrap();
        assert_eq!(cut["error"]["type"], "incomplete_stream");
    ...
    /// The answer a transcript comes to, and the events it emitted.
    fn streamed(transcript: &str) -> (Value, Vec<Value>) {
        let mut deltas = Deltas::new(Vec::new());
        let mut assembly = Assembly::default();
        for frame in frames(transcript) {
            assembly.apply(&frame, &mut deltas);
        }
        let envelope = answer(Outcome::Completed, assembly, "m", &mut deltas);
        (serde_json::from_str(&envelope).unwrap(), deltas.into_sink())
    }

    #[test]
    fn a_stream_that_ends_before_its_final_response_is_refused_as_incomplete() {
        let transcript = [
            event("response.created", json!({"response": {"status": "in_progress"}})),
            event("response.output_item.added", json!({"output_index": 0, "item": {"type": "message"}})),
            event("response.output_text.delta", json!({"output_index": 0, "content_index": 0, "delta": "Let me "})),
            event("response.output_item.added", json!({"output_index": 1, "item": {"type": "function_call", "id": "fc_1", "call_id": "call_1", "name": "files.read"}})),
            event("response.function_call_arguments.delta", json!({"output_index": 1, "item_id": "fc_1", "delta": "{\"pa"})),
        ]
        .concat();

        let (envelope, events) = streamed(&transcript);
        assert_eq!(envelope["status"], 502);
        assert_eq!(envelope["error"]["type"], "incomplete_stream");
        assert!(envelope.get("data").is_none());

        let last = events.last().unwrap();
        assert_eq!(last["type"], "error");
        assert_eq!(last["error"]["type"], "incomplete_stream");
        assert!(events.iter().all(|e| e["type"] != "stop"));
    }

    #[test]
    fn two_function_calls_added_at_one_output_index_stay_two_calls() {
        let added = |item: &str, call: &str, name: &str| {
            event("response.output_item.added", json!({"output_index": 0,
                "item": {"type": "function_call", "id": item, "call_id": call, "name": name, "arguments": ""}}))
        };
        let arguments = |item: &str, delta: &str| {
            event("response.function_call_arguments.delta", json!({"output_index": 0, "item_id": item, "delta": delta}))
        };
        let whole = json!({
            "status": "completed",
            "output": [
                {"type": "function_call", "id": "fc_a", "call_id": "call_a", "name": "files.read", "arguments": "{\"path\":\"a\"}"},
                {"type": "function_call", "id": "fc_b", "call_id": "call_b", "name": "files.list", "arguments": "{\"dir\":\"b\"}"}
            ]
        });
        let transcript = [
            added("fc_a", "call_a", "files.read"),
            arguments("fc_a", "{\"path\":\"a\"}"),
            added("fc_b", "call_b", "files.list"),
            arguments("fc_b", "{\"dir\":"),
            arguments("fc_b", "\"b\"}"),
            event("response.output_item.done", json!({"output_index": 0, "item": {"type": "function_call", "id": "fc_b"}})),
            event("response.completed", json!({"response": whole.clone()})),
        ]
        .concat();
        ...
        let (envelope, events) = streamed(&transcript);
        assert_eq!(envelope["data"], normalize(&whole, "m"));
        assert_eq!(envelope["data"]["content"][0]["id"], "call_a");
        assert_eq!(envelope["data"]["content"][1]["id"], "call_b");

        let calls: Vec<&Value> = events.iter().filter(|e| e["type"] != "usage" && e["type"] != "stop").collect();
        assert_eq!(
            calls,
            vec![
                &json!({"type": "tool_call.start", "index": 0, "id": "call_a", "name": "files.read"}),
                &json!({"type": "tool_call.delta", "index": 0, "arguments": "{\"path\":\"a\"}"}),
                &json!({"type": "tool_call.end", "index": 0}),
                &json!({"type": "tool_call.start", "index": 1, "id": "call_b", "name": "files.list"}),
                &json!({"type": "tool_call.delta", "index": 1, "arguments": "{\"dir\":\"b\"}"}),
                &json!({"type": "tool_call.end", "index": 1}),
            ]
        );
    }
