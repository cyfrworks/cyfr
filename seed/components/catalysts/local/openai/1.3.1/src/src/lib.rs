...
pub(crate) const BASE_URL: &str = "https://api.openai.com";
...
fn models_get(api_key: &str, params: &Value) -> Result<String, String> {
    let model_id = match params.get("model_id").and_then(Value::as_str) {
        Some(model) if chat::model_id(model) => ids::segment(model),
        Some(model) => return Ok(chat::unknown_model(model)),
        None => return Ok(format_error(400, "invalid_request", "Missing 'model_id' in params")),
    };
...
mod tests {
    ...
    fn a_name_off_the_id_grammar_is_refused_before_any_request() {
        for name in HOSTILE {
            ...
            let model: Value =
                serde_json::from_str(&models_get("key", &json!({"model_id": name})).unwrap()).unwrap();
            assert_eq!(model["error"]["type"], "unknown_model", "{name:?}");
        ...
        assert!(chat::model_id("ft:gpt-4o-mini-2024-07-18:org::abc"));
        assert_eq!(path_id(&json!({"file_id": "file-abc_1"}), "file_id"), Ok("file-abc_1".to_string()));
        assert!(path_id(&json!({"file_id": "ft:x"}), "file_id").is_err());
    }
}
