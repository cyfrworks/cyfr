...
| Everything else (`apps/arca/**` including the tenancy migrations under `apps/arca/priv/repo/migrations/`, `apps/cyfr/lib/{emissary*,prism*,compendium,aqua,cyfr,grimoire,crucible}/`, `apps/{prima,locus,opus}/`, web UIs, docs) | Apache-2.0 | Apache by default; the schema DDL every self-hoster must run is not gated. |
...
Apache-2.0. The control plane's Apache-licensed code is not independently
buildable: it calls into Sanctum throughout (every tenancy and consent
decision is Sanctum's), so running CYFR means running the FSL part under
