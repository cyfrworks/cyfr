...
| Everything else (`apps/arca/**` including the tenancy migrations under `apps/arca/priv/repo/migrations/`, `apps/cyfr/lib/{emissary*,prism*,compendium,aqua,cyfr,grimoire}/`, `apps/{prima,locus,opus}/`, web UIs, docs) | Apache-2.0 | Apache by default; the schema DDL every self-hoster must run is not gated. |
