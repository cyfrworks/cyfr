...
impl PolicyGuard {
    /// Build from the policy map and the pre-policy tool definitions (for
    /// annotations). `policy` must be a JSON object.
    pub fn new(policy: &Value, pre_policy_tools: &[Value]) -> Self {
        ...
        PolicyGuard { policy: policy.clone(), annotations }
    }
