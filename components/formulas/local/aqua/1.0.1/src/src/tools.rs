...
pub fn build_tool_definitions(visible_tools: Option<&[String]>, sub_agents: &[SubAgentDef]) -> Vec<Value> {
    ...
    // Virtual HTTP tools — wrappers around catalyst:local.http
    if virtual_tool_allowed(visible_tools, "http") {
        tools.push(json!({
            "name": "http_read",
            "description": "Read a web page as clean markdown. Works with localhost, internal docs, and external URLs.",
            "input_schema": {
                "type": "object",
                "required": ["url"],
                "properties": {
                    "url": {"type": "string", "description": "URL to read (e.g. http://localhost:3000/docs)"}
                }
            }
        }));
        ...
        tools.push(json!({
            "name": "http_fetch",
            "description": "Raw HTTP request. Use for APIs, webhooks, or when you need full response details.",
            "input_schema": {
                "type": "object",
                "required": ["url"],
                "properties": {
...
pub fn dispatch_tool(
    ...
    match tool_name {
        ...
        "http_read" => dispatch_http_op("read", args),
        "http_fetch" => dispatch_http_op("fetch", args),
