...
fn handle_request(input: &str) -> Result<String, String> {
    ...
    let files_ref = parsed
        .get("files_ref")
        .and_then(|v| v.as_str())
        .unwrap_or("catalyst:local.files:0.1.0");
    ...
    // --- Build system prompt with CYFR context ---
    let system_prompt = context::build_system_prompt(project_path, custom_system);
