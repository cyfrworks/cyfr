...
│   └── formula/     #   Composition interface (invoke)
...
| Call MCP tools (via `invoke::call`) | No | No | Yes (optional) |
...
### MCP Tool Access via `invoke::call` (Formula only)

MCP tools are accessed through the same `invoke::call` host function used for sub-component invocation. There is no separate `cyfr:mcp/tools` WIT interface — the Elixir FormulaHandler detects the `{"tool": ..., "action": ...}` format and routes to MCP dispatch.
...
> **Host Policy Required**: Formulas calling MCP tools MUST have Host Policy defining `allowed_tools`. **Deny-by-default**: unlisted tools are blocked.
>
...
- `setup.policy` is available for all types. Generic fields (`timeout`, `max_memory_bytes`, `rate_limit`, etc.) apply uniformly. Catalyst-specific fields (`allowed_domains`, `allowed_methods`, `allowed_paths`) only matter for catalysts. Formula-specific fields (`allowed_tools`) only matter for formulas calling MCP tools via `invoke::call`.
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `allowed_tools` | string[] | `[]` (deny-all) | MCP tools the formula can call via `invoke::call`. Only needed if the formula calls MCP tools. |
...
- Formulas don't use the `wasi` field — their single import is `cyfr:formula/invoke`. MCP tool access goes through `invoke::call` with `{"tool": ..., "action": ...}` format, controlled by `setup.policy.allowed_tools`.
...
**Cargo.toml**: Package metadata uses `cyfr:formula` and world `"formula"`. No extra dependencies needed — `invoke` is built into the formula world:
...
A **Brain Formula** uses LLM reasoning combined with MCP tools (via `invoke::call`) to dynamically discover, generate, and invoke components at runtime.
...
User Request → Brain calls LLM Catalyst for reasoning
  ...
  → If not found anywhere → Brain asks LLM to generate source
    → mcp.call("build.compile", {source: "...", language: "rust", target: "reagent"})
