defmodule Arca.DependencyStorage do
  ...
  Accepts a context, component_id, and a list of dependency maps, each containing:
  ...
  """
  @spec get_dependencies(Context.t(), String.t()) :: {:ok, [map()]} | {:error, term()}
  def get_dependencies(%Context{} = ctx, component_id) when is_binary(component_id) do
    query =
      from(d in @table,
