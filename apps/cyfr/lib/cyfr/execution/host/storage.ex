...
defmodule Cyfr.Execution.Host.Storage do
  @moduledoc """
  ...
  `Cyfr.Execution.Attempt` runs it with `run/2` once the call is checked,
  ...
  | Operation | `args` | `ok` |
  |---|---|---|
  | `storage` | `action` (`read`, `write`, `append`, `list`, `delete`, `exists`), `path`, optional `content` | the answer's members (`Cyfr.Execution.GuestStorage`) |
  ...
  alias Cyfr.Execution.{Artifacts, GuestStorage}
  ...
  def run({:fetch_artifact, digest}, %{digest: digest} = attempt) when is_binary(digest) do
    case Artifacts.fetch(attempt.admission_ctx, digest, attempt.component_ref) do
      ...
      {:error, reason} ->
        Logger.error(
          "[Cyfr.Execution.Host.Storage] artifact #{digest} of #{attempt.component_ref} " <>
  ...
  def run({:fetch_artifact, _digest}, attempt) do
    Logger.warning(
      "[Cyfr.Execution.Host.Storage] refused an artifact that is not #{attempt.component_ref}'s"
    )
