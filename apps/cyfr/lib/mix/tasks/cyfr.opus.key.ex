...
defmodule Mix.Tasks.Cyfr.Opus.Key do
  @shortdoc "Print a worker service's key, derived from CYFR_OPUS_KEY"
  ...
  the service id, derived from the root in `CYFR_OPUS_KEY`.

      CYFR_OPUS_KEY=… mix cyfr.opus.key opus
  ...
  def run([service_id]) do
    with {:ok, root} <- root(System.get_env("CYFR_OPUS_KEY")),
  ...
  def run(_args), do: Mix.raise("usage: mix cyfr.opus.key <service_id>")

  defp root(nil), do: {:error, "CYFR_OPUS_KEY is not set"}

  defp root(text) do
    case WorkerAuth.decode_root(text) do
      {:ok, root} -> {:ok, root}
      :error -> {:error, "CYFR_OPUS_KEY is not 64 hexadecimal digits"}
    end
  end
