defmodule Sanctum.Crypto do
  ...
  defp get_iterations do
    Application.get_env(:cyfr, :pbkdf2_iterations, @default_iterations)
  end
    ...
    case encryption_key(opts) do
      {:ok, key} ->
        ...
        case :crypto.crypto_one_time_aead(:aes_256_gcm, key, iv, ciphertext, <<>>, tag, false) do
          ...
          :error ->
            Logger.error("Decryption failed - possible key mismatch or corrupted data")
  ...
  def decrypt(encrypted, _opts) when is_binary(encrypted) do
    Logger.error("Decryption failed - data too short (#{byte_size(encrypted)} bytes)")
