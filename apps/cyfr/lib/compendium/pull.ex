...
defmodule Compendium.Pull do
  ...
  def pull_shipped(%Context{} = ctx, reference) when is_binary(reference) do
    with {:ok, %Prima.ComponentRef{} = cref} <- Prima.ComponentRef.parse(reference),
         :ok <- local_ref(cref),
      ...
      pulled = %Prima.ComponentRef{cref | version: version}
      {:ok, %{status: "pulled", component_ref: Prima.ComponentRef.to_string(pulled)}}
    end
  end

  defp local_ref(%Prima.ComponentRef{namespace: namespace}) do
  ...
  # else the newest.
  defp shipped_version(%Prima.ComponentRef{} = cref) do
  ...
  def missing_deps(%Context{} = ctx, component, opts \\ []) when is_map(component) do
    manifest = Prima.Manifest.decode(Map.get(component, :manifest))
  ...
  def oci_reference_for(reference) when is_binary(reference) do
    case Prima.ComponentRef.parse(reference) do
      {:ok, %Prima.ComponentRef{namespace: ns} = cref} ->
  ...
  defp to_oci_ref(%Prima.ComponentRef{version: nil} = cref) do
  ...
  defp to_oci_ref(%Prima.ComponentRef{} = cref) do
    registry = Compendium.RegistryHost.canonical_host()
