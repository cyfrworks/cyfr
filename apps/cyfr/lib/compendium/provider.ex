...
defmodule Compendium.Provider do
  @moduledoc """
      ...
      [reference, path] when path != "" ->
        ...
        with {:ok, asset_segments} <- validate_asset_path(path),
          ...
          case Arca.get(Sanctum.Context.actor(ctx), asset_path) do
            ...
            {:error, reason} ->
              Logger.warning(
                "[Compendium.Provider] Asset not found: #{rest} (#{inspect(reason)})"
              )
  ...
  def tools do
    [
      Compendium.Providers.Component.definition(),
      Compendium.Providers.Aqua.definition(),
      Compendium.Providers.Registry.definition()
    ]
  end
  ...
  def handle("component", ctx, args),
    do: ctx |> Compendium.Providers.Component.handle(args) |> index("Component index")

  def handle("aqua", ctx, args),
    do: ctx |> Compendium.Providers.Aqua.handle(args) |> index("Agent index")

  def handle("registry", ctx, args),
    do: ctx |> Compendium.Providers.Registry.handle(args) |> index("Component index")
