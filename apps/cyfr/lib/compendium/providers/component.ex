...
defmodule Compendium.Providers.Component do
  @moduledoc """
  ...
  Extracted from `Compendium.Provider`; behaviour preserved exactly.
  """
  ...
  # handler they gate; Compendium.Provider assembles its roster from these.
  def definition do
    ...
    Operation.tool(
      [
        ...
        # (`Compendium.Provider.resource_templates/0`): the gate decides it
          ...
          case oci_reference do
            ...
            {:ok, ref} ->
              ...
              case result do
                ...
                {:error, reason} ->
                  Logger.error("[Compendium.Providers.Component] Pull failed: #{inspect(reason)}")
                  broadcast_progress(ctx, progress_id, :error, "Pull failed")
              end
          ...
          {:ok, cref} ->
            with :ok <- refuse_non_local_push(cref),
              ...
              case Compendium.OCI.Client.push(ctx, reference, registry) do
                ...
                {:error, reason} ->
                  Logger.error("[Compendium.Providers.Component] Push failed: #{inspect(reason)}")
  ...
  def handle(%Context{} = ctx, %{"action" => "register"} = args) do
    ...
    case Compendium.AutoIndexer.scan(ctx: ctx) do
      ...
      {:error, {:discovery_failed, reason}} ->
        ...
        Logger.error("[Compendium.Providers.Component] component scan failed: #{inspect(reason)}")
  ...
  def handle(%Context{} = ctx, %{"action" => "list"} = args) do
    ...
    with {:ok, result} <- Registry.search(ctx, filters),
      ...
      {:error, reason} ->
        Logger.error("[Compendium.Providers.Component] component.list failed: #{inspect(reason)}")
    ...
    case Prima.ComponentRef.parse(reference) do
      ...
      {:ok, cref} ->
        case Registry.delete(ctx, cref.name, cref.version, cref.namespace) do
          ...
          {:error, reason} ->
            Logger.error(
              "[Compendium.Providers.Component] component.delete failed: #{inspect(reason)}"
            )
  ...
  def handle(%Context{} = ctx, %{"action" => "reset", "reference" => reference}) do
    with {:ok, %{version: version} = cref} when is_binary(version) <-
      ...
      case Registry.reset(ctx, cref.name, cref.version, cref.namespace) do
        ...
        {:error, reason} ->
          Logger.error(
            "[Compendium.Providers.Component] component.reset failed: #{inspect(reason)}"
          )
  ...
  def handle(%Context{} = ctx, %{"action" => "status", "reference" => reference}) do
    with {:ok, %{version: version} = cref} when is_binary(version) <-
      ...
      {:error, reason} ->
        Logger.error(
          "[Compendium.Providers.Component] component.status failed: #{inspect(reason)}"
        )
  ...
  def handle(%Context{} = ctx, %{"action" => "status"}) do
    case Compendium.Provenance.overview(ctx) do
      {:error, reason} ->
        Logger.error(
          "[Compendium.Providers.Component] status overview read failed: #{inspect(reason)}"
        )
  ...
  def handle(%Context{} = ctx, %{"action" => "get_blob", "digest" => digest}) do
    case Registry.get_blob(ctx, digest) do
      ...
      {:error, reason} ->
        Logger.error("[Compendium.Providers.Component] Failed to get blob: #{inspect(reason)}")
  ...
  defp check_register_deps(ctx, components, register_id) do
    ...
    e in [
      ...
      Logger.error(
        "[Compendium.Providers.Component] Exception in check_register_deps: #{Exception.message(e)}"
      )
  ...
  defp auto_pull_published_deps(ctx, published_missing, register_id) do
    ...
    on_progress = fn
      ref, :pulling ->
        Logger.info(
          "[Compendium.Providers.Component] Auto-pulling dependency after register: #{ref}"
        )
  ...
  defp record_push(ctx, reference, result) do
    ...
    Logger.info(
      "[Compendium.Providers.Component] pushed #{reference} as #{inspect(oci_reference)} " <>
  ...
  defp publish_progress(ctx, subject, phase, message) do
    ...
    case Cyfr.Bus.broadcast_progress(actor, step) do
      ...
      {:error, reason} ->
        Logger.warning(
          "[Compendium.Providers.Component] progress not published: #{inspect(reason)}"
        )
    end
  ...
  defp shipped_pull_error(reference, reason) do
    Logger.error(
      "[Compendium.Providers.Component] shipped pull of #{reference} failed: #{inspect(reason)}"
    )
  ...
  defp oci_pull(ctx, reference, anonymous?) do
    if anonymous? do
      Logger.warning(
        "[Compendium.Providers.Component] No credentials for #{Compendium.RegistryHost.canonical_host()} — " <>
    ...
    case Compendium.OCI.Client.pull(ctx, reference) do
      {:ok, result} ->
        if result[:warning],
          do: Logger.warning("[Compendium.Providers.Component] #{result.warning}")
  ...
  defp maybe_auto_pull_oci_deps(ctx, _oci_reference, result) do
    ...
    e in [
      ...
      Logger.warning(
        "[Compendium.Providers.Component] Exception in auto-pull OCI deps: #{Exception.message(e)}"
      )
  ...
  defp auto_pull_deps(ctx, deps, result) do
    ...
    %{pulled: pulled, failed: failed} =
      Compendium.Pull.ensure_published_deps(ctx, refs,
        on_progress: fn
          ref, :pulling ->
            Logger.info("[Compendium.Providers.Component] Auto-pulling dependency: #{ref}")
  ...
  defp decode_manifest(value, ref) do
    case Prima.Manifest.decode_strict(value) do
      ...
      {:error, :malformed_manifest} ->
        Logger.warning("[Compendium.Providers.Component] manifest malformed: #{ref}")
        %{}
    end
  end
