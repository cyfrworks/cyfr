...
defmodule Compendium.AthanorSeeder do
  ...
  """
  @spec sync(String.t() | %{id: String.t()}) :: {:ok, report()} | {:error, term()}
  def sync(%{id: athanor_id}), do: sync(athanor_id)

  def sync(athanor_id) when is_binary(athanor_id) and athanor_id != "" do
    ...
    with :ok <- bundle_present(ctx),
         {:ok, report} <- sync_versions(ctx) do
  ...
  defp sync_versions(ctx) do
    Enum.reduce_while(Bundle.type_plurals(), {:ok, empty_report()}, fn type_plural, {:ok, acc} ->
      ...
      case bundle_versions(ctx, src) do
        {:ok, versions} ->
          case sync_type(ctx, type_plural, src, dest, versions, acc) do
            {:ok, _} = ok -> {:cont, ok}
  ...
  defp bundle_versions(ctx, src) do
    with {:ok, leaves} <- Arca.list_recursive(ctx, src) do
      versions =
        leaves
        |> Enum.map(&Enum.drop(&1, length(src)))
        |> Enum.filter(&match?([_name, _version | _], &1))
        |> Enum.map(fn [name, version | _] -> {name, version} end)
        |> Enum.uniq()
  ...
  defp sync_type(ctx, type_plural, src, dest, versions, acc) do
    Enum.reduce_while(versions, {:ok, acc}, fn {name, version}, {:ok, acc} ->
      ...
      case Arca.list_typed(ctx, dest_dir) do
        ...
        {:ok, _entries} ->
          ...
          key = if drifted?(ctx, src ++ [name, version], dest_dir), do: :modified, else: :present
