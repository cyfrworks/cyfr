...
defmodule Crucible.Provider do
  ...
  def resource_templates do
    [
      %{
        uriTemplate: "crucible://executions/{id}",
        name: "Execution State",
        description: "Get execution state by ID",
        mimeType: Prima.MediaType.json()
      },
      %{
        uriTemplate: "crucible://executions/{id}/logs",
        name: "Execution Logs",
  ...
  # external plane); `Crucible.Record.get/2` supplies the tenant
  # scoping under the caller's own context.
  defp read_resource(%Context{} = ctx, "crucible://executions/" <> rest) do
  ...
  # Parse the URI path after "crucible://executions/"
  ...
  defp parse_execution_uri(path) do
    case String.split(path, "/", parts: 2) do
      ...
      _ ->
        {:error,
         {:invalid_argument,
          "Invalid execution URI format. Expected: crucible://executions/{id} or crucible://executions/{id}/logs"}}
    end
  end
    ...
    case Record.get(ctx, exec_id) do
      {:ok, record} ->
        ...
        case Jason.encode(content, pretty: true) do
          ...
          {:error, err} ->
            Logger.error("[Crucible.Provider] Failed to encode execution record: #{inspect(err)}")
  ...
  def tools do
    ...
    # chain's authority (`Crucible.run_child/5`), and an approved
    ...
    [
      Operation.tool(
        [
          ...
          # The admission of an MCP `resources/read` of a crucible:// URI
          # (`resource_templates/0`).
          Operation.new(
            "execution",
            "read_resource",
            "Read a crucible://executions/ resource",
            [
              Arg.new("uri", :string,
                required: true,
                description:
  ...
  def handle(
    ...
    with :ok <- check_chain_scope(ctx, execution_id, args) do
      case Crucible.cancel(ctx, execution_id) do
        {:ok, result} ->
          {:ok, result}
        ...
        {:error, reason} ->
          Logger.error("[Crucible.Provider] Failed to cancel execution: #{inspect(reason)}")
  ...
  def handle("execution", %Context{} = ctx, %{"action" => "force_release"}) do
    Logger.warning("[Crucible.Provider] Force release triggered by user=#{ctx.user_id}")
  ...
  defp start_root("run_stream", ctx, args) do
    ...
    case Task.Supervisor.start_child(Crucible.TaskSupervisor, fn ->
           Prima.LoggerContext.restore(logger_metadata)

           case Registry.register(Crucible.Registry, execution_id, :running) do
             {:ok, _} ->
               run_root_formatted(ctx, reference, input, opts, args)

             {:error, reason} ->
               Logger.error(
                 "[Crucible.Provider] Failed to register execution #{execution_id}, aborting: #{inspect(reason)}"
               )
           end
         end) do
      {:ok, _pid} ->
      ...
      {:error, reason} ->
        Logger.error(
          "[Crucible.Provider] Failed to spawn execution #{execution_id}: #{inspect(reason)}"
        )
  ...
  defp run_root_formatted(ctx, reference, input, opts, args) do
    ...
    case Crucible.run_root(ctx, selector, reference, input, opts) do
