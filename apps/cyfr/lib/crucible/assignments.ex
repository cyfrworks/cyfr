...
defmodule Crucible.Assignments do
  @moduledoc """
  ...
  signs it with the assign key (`Crucible.Keys`). The runner presents
  the token at attach (`Crucible.Host`), which verifies it, checks
  it names the attempt its header does and claims the attempt.

  An assignment is issued under the current generation
  (`Crucible.Keys.generation/0`), to the worker service and the boot
  ...
  It also carries **this member**: `Crucible.Keys.member/0`, and the
  ...
  (`Crucible.Keys.generation/0`), and `{:error, reason}` when what
