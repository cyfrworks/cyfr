...
defmodule Crucible.LeaseWatch do
  @moduledoc """
  The lease keeper of an execution held outside the engine — a turn root
  (`Crucible.TurnRoot`) or an outbound tool call in flight: a
  ...
  (`Crucible.Record.renew_lease/2`): a renewal the store refuses,
  ...
  defp initial_until(execution_id) do
    ...
    e ->
      Logger.warning(
        "[Crucible.LeaseWatch] lease start for #{execution_id} could not read the " <>
