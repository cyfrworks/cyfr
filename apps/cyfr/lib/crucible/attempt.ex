...
defmodule Crucible.Attempt do
  ...
  reaches it only through host calls (`Crucible.Host`), which verify
  the call before they reach here.

  `Crucible.Admission` opens it once the run's row is admitted,
  registered under the execution's id (with the attempt id as its value)
  in `Crucible.Attempt.Registry` and supervised by
  `Crucible.Attempt.Supervisor`. The process that opens it is its
  waiter (`Crucible.Dispatch.await/2`) until it hands the attempt to
  the runner that claimed it (`hand_over/1`), as a formula's child run in
  its parent's runner is handed over. It holds:

  - the close state (`Crucible.Close`): the admitted row, the
  ...
  - what its guest's children and catalog tools are decided under
    (`Crucible.Host.Children`): the run's authority, its root, the
  ...
  `Crucible.Close` in this process with the full set, gives back what
  ...
  its row (`Crucible.Lapse`). A waiter whose attempt stops without closing
  closes the run lost (`Crucible.Close.lost/1`), which writes nothing
  ...
  lost close writes nothing (`Crucible.Close.lost/1`); the rows are
  ...
  alias Prima.Delta
  alias Crucible.{Charge, Close, Emit, Host, Lapse, StepSpans}
  alias Prima.Outcome
  alias Prima.Slots
  ...
  assignment is signed from, `t:Crucible.Assignments.admitted/0`,
  ...
  `Crucible.Slots`, in the attempt's slot holder). Answers `:ok`
  ...
  it back (`c:Prima.HostAPI.release_child/2`), which `Crucible.Host`
  ...
  `Crucible.Registry` as dispatched to its worker service. Answers
  ...
  (`Crucible.StepSpans.guest_started/1`). A selection the loader
  ...
  signed from (`t:Crucible.Assignments.admitted/0`, the `:assignment`
  ...
  @spec admitted(String.t(), holder()) ::
          {:ok, %{assignment: Crucible.Assignments.admitted(), secrets: map()}}
  ...
  exit (`Crucible.Host.runner_exited/2`), so nothing of the run is
  ...
  (`Crucible.Close.lost/1`) and tells its waiter the result, which
  ...
  def note_unreaped(tenant, execution_id) when is_binary(tenant) do
    case Slots.note_unreaped(@slots, tenant, execution_id) do
      ...
      {:error, :unavailable} ->
        Logger.error(
          "[Crucible.Attempt] unreaped kill of #{inspect(execution_id)} for tenant " <>
  ...
  def handle_call(message, _from, state) do
    Logger.error(
      "[Crucible.Attempt] #{state.execution_id} refused an unknown call " <>
  ...
  def handle_cast(:stop_ended, state) do
    case row_live(state) do
      ...
      :unavailable ->
        Logger.error(
          "[Crucible.Attempt] #{state.execution_id} was told its row ended, and could " <>
            "not read it"
        )
  ...
  def handle_cast(message, state) do
    Logger.error(
      "[Crucible.Attempt] #{state.execution_id} dropped an unknown cast " <>
        inspect(message_shape(message))
    )
  ...
  def handle_info({:EXIT, holder, reason}, %__MODULE__{slot: {:held, holder}} = state) do
    Logger.error(
      "[Crucible.Attempt] #{state.execution_id}'s slot holder ended (" <>
  ...
  defp give_back_charge(state) do
    ...
    :exit, reason ->
      Logger.error(
        "[Crucible.Attempt] #{state.execution_id}'s charge was not given back: " <>
          inspect(reason)
      )
  end
  ...
  defp kill_runner(state) do
    killed = Crucible.WorkerClient.kill(state.worker, state.execution_id)
  ...
  defp refuse_outcome(state) do
    Logger.error(
      "[Crucible.Attempt] #{state.execution_id} refused an outcome naming another attempt"
    )
  ...
  defp flush(state) do
    case Arca.ExecutionAttempts.standing?(
      ...
      _retired ->
        Logger.warning(
          "[Crucible.Attempt] #{state.execution_id} closed with held text its grant " <>
  ...
  defp emit_delta(state, emit, %Delta{} = delta) do
    ...
    exception ->
      Logger.error(
        "[Crucible.Attempt] #{state.execution_id} emit raised: " <>
  ...
  defp take_rate(state, "http:" <> ref = bucket) when ref == state.component_ref do
    case Crucible.Rates.check(Context.actor(state.ctx), bucket, %{
  ...
  defp resolve_token(%__MODULE__{authority: %Authority{resources: resources}} = state, provider) do
    ...
    exception ->
      Logger.error(
        "[Crucible.Attempt] #{state.execution_id} token dispense raised: " <>
          Exception.format(:error, exception, __STACKTRACE__)
      )
