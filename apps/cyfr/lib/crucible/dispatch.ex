...
defmodule Crucible.Dispatch do
  @moduledoc """
  ...
  row's runner and the assignment's audience. In order:
    ...
    1. the run is admitted (`Crucible.Admission.admit/4`): its row
       and its `Crucible.Attempt`;
    2. the calling process, the run's waiter, is registered under the
       execution's id in `Crucible.Registry` as `:admitted`: it has
       started nothing yet;
    3. the attempt takes the run's execution slot — `:child` for a run
       ...
       (`Crucible.Attempt.take_slot/3`);
    4. the waiter's registration becomes `{:dispatched, endpoint}`, the
       assignment is signed (`Crucible.Assignments.issue/1`), its
       ...
       worker service (`Crucible.WorkerClient.start/4`) with the
  ...
  has the run closed lost (`Crucible.Close.lost/1`), which answers
  ...
  (`Crucible.Attempt.hand_over/1`), so nothing on CYFR waits for it,
  ...
  alias Crucible.{Admission, Assignments, Attempt, Cascade, Charge, Close, Keys, Record}
  alias Crucible.{WorkerClient, WorkerWatch}
  ...
  `Crucible.Admission.admit/4`'s (without `:service_id`, `:boot_id`
  ...
  `Crucible.Admission.admit/4`'s, with `:service_id`, `:boot_id` and
  ...
  as `Crucible.Close` answered. An attempt that stops without closing
  its run is closed lost with `close` (`Crucible.Close.lost/1`).
  """
  ...
  (`Crucible.Record.cancel/3`, with `opts[:restart_required]`)
  ...
  (`Crucible.Cascade`), what runs it is stopped (`stop/2`) and the
  ...
  def cancel(%Context{} = ctx, execution_id, opts \\ []) do
    ...
    case Record.cancel(ctx, execution_id, Keyword.take(opts, [:restart_required])) do
      {:ok, record} ->
        ...
        Crucible.Telemetry.execute_cancelled(record, ctx.user_id)
  ...
  (`Crucible.Attempt.stop_ended/1`): one no runner has attached to
  ...
  id in `Crucible.Registry`. A run whose waiter has not started it
  ...
  run (`Crucible.Attempt.note_unreaped/2`) once for each run, and
  ...
  skipped. The status is the one `Crucible.WorkerWatch` heard
  ...
  defp dispatch(admitted, endpoint, input, opts) do
    ...
    after
      if registered?, do: Registry.unregister(Crucible.Registry, admitted.execution_id)
    end
  end
  ...
  # if no runner attached (`Crucible.Attempt.refuse/2`), since the
    ...
    with {:signed, {:ok, issued}} <- {:signed, Assignments.issue(admitted.assignment)},
      ...
      {:error, :lost} ->
        case Attempt.refuse(admitted.attempt, "the execution worker did not answer the start") do
          :attached ->
            Logger.warning(
              "[Crucible.Dispatch] #{admitted.execution_id}: the start's answer was " <>
                "lost, and its runner attached"
            )

          :closed ->
            Logger.error(
              "[Crucible.Dispatch] #{admitted.execution_id} was not started: the " <>
  ...
  defp refuse(admitted, sentence, reason) do
    Logger.error(
      "[Crucible.Dispatch] #{admitted.execution_id} was not started: #{inspect(reason)}"
    )
  ...
  defp register_waiter(execution_id) do
    case Registry.register(Crucible.Registry, execution_id, :admitted) do
      {:ok, _owner} ->
        true

      {:error, {:already_registered, owner}} when owner == self() ->
        Registry.update_value(Crucible.Registry, execution_id, fn _ -> :admitted end)
        false
  ...
  defp mark_dispatched(execution_id, endpoint) do
    Registry.update_value(Crucible.Registry, execution_id, fn _ ->
      {:dispatched, endpoint}
    end)
