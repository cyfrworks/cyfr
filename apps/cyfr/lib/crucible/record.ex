...
defmodule Crucible.Record do
  ...
  defstruct [
    ...
    # belongs to; a turn root is admitted by `Crucible.TurnRoot`.
    :kind,
    :turn_id,
    # The schedule this root runs for (`Crucible.Schedules`); nil otherwise.
    :schedule_id,
  ...
  defp publish(%__MODULE__{} = record, type, seq, data) when is_integer(seq) do
    _ = Crucible.Events.publish(record.id, record, type, seq, data)
    :ok
  end
  ...
  def renew_lease(execution_id, attempt) when is_binary(execution_id) and is_binary(attempt) do
    ...
    e ->
      Logger.warning(
        "[Crucible.Record] lease renewal failed for #{execution_id}: #{Exception.message(e)}"
      )
  ...
  defp result_lost(ctx, %__MODULE__{} = record, reason) do
    Logger.error(
      "[Crucible.Record] result of #{record.id} not retained: #{inspect(reason)}; " <>
  ...
  def list(%Context{} = ctx, opts \\ []) do
    ...
    # dead — `Crucible.ArchiveWatch` has one written for exactly
  ...
  defp parse_json_or_nil(json) when is_binary(json) do
    case Jason.decode(json) do
      ...
      {:error, _reason} ->
        Logger.warning(
          "[Crucible.Record] Failed to parse stored JSON (#{byte_size(json)} bytes), returning nil"
        )
  ...
  defp parse_status(unknown) do
    Logger.warning(
      "[Crucible.Record] Unrecognized status: #{inspect(unknown)}, treating as :unknown"
    )
  ...
  defp parse_component_type(type_str) when is_binary(type_str) do
    case executable_type(type_str) do
      ...
      :error ->
        Logger.warning(
          "[Crucible.Record] Unknown component type: #{inspect(type_str)}, defaulting to :reagent"
        )
