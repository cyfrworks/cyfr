...
defmodule Crucible.ArchiveWatch do
  @moduledoc """
  ...
  defp cancel_running(athanor_id) do
    if Crucible.available?() do
      ...
      case Crucible.list(ctx, status: :running, limit: @cancel_limit) do
        {:ok, running} ->
          Enum.each(running, fn %{id: id} -> Crucible.cancel(ctx, id) end)

        {:error, reason} ->
          Logger.warning(
            "[Crucible.ArchiveWatch] #{athanor_id}: running executions not listed " <>
    ...
    e ->
      Logger.warning("[Crucible.ArchiveWatch] cancel on archive failed (#{Exception.message(e)})")
    ...
    # lease.
    kind, reason ->
      Logger.warning("[Crucible.ArchiveWatch] cancel on archive #{kind}: #{inspect(reason)}")
