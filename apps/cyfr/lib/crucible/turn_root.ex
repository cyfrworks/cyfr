...
defmodule Crucible.TurnRoot do
  @moduledoc """
  ...
  `Crucible.Slots`), keyed by the athanor. It is taken together with
  the registry entry that lets a cancel find the holder
  (`Crucible.Dispatch.stop/2`), and both are given back together. A
  ...
  alias Crucible.{Admission, LeaseWatch, Record}
  alias Prima.Slots
  ...
  `Crucible.Registry` (a process registered already keeps its entry).
  """
  ...
  defp give_back(%{slot: ref, registered: registered?, execution_id: execution_id}) do
    ...
    if registered?, do: Registry.unregister(Crucible.Registry, execution_id)
    :ok
  end
