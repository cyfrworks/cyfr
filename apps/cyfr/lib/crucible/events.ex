...
defmodule Crucible.Events do
  ...
  defp dropped(execution_id, type) do
    Logger.error(
      "[Crucible.Events] dropping #{type} event for #{execution_id}: " <>
  ...
  defp broadcast(execution_id, athanor_id, event) do
    ...
    case Cyfr.Bus.broadcast(actor, Cyfr.Bus.execution_events(actor, execution_id), payload) do
      ...
      {:error, reason} ->
        Logger.error(
          "[Crucible.Events] PubSub broadcast failed for #{execution_id}: #{inspect(reason)}"
        )
  ...
  def flush(execution_id) do
    case Registry.lookup(Crucible.Events.Registry, execution_id) do
  ...
  defp via(execution_id) do
    {:via, Registry, {Crucible.Events.Registry, execution_id}}
  end
