...
defmodule Crucible.Rates do
  ...
  `Crucible.Slots` holds (`Prima.Slots`): nothing here holds,
  ...
  ## Usage
      ...
      case Crucible.Rates.check(actor, "stripe-catalyst", %{
  ...
  ## Examples
      ...
      iex> Crucible.Rates.check(actor, "component", %{rate_limit: %{requests: 10, window: "1m"}})
      {:ok, 9}
  ...
  defp unavailable(reason, operation) do
    Logger.warning(
      "[Crucible.Rates] the rate authority could not answer #{operation} " <>
  ...
  defp missing_tenant(operation) do
    Logger.warning(
      "[Crucible.Rates] no athanor resolved during #{operation} — rejecting to prevent " <>
  ...
  defp parse_window(window) do
    case Prima.Limits.parse_duration(window) do
      ...
      {:error, reason} ->
        Logger.warning("[Crucible.Rates] invalid rate-limit window #{inspect(window)}: #{reason}")
