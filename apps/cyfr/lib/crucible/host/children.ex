...
defmodule Crucible.Host.Children do
  @moduledoc """
  ...
  `Crucible.Host` verifies the call, decodes its body
  (`operation/2`) and checks its nonce before `call/2` acts. Both act under
  what CYFR holds for the calling attempt (`Crucible.Attempt`): the
  ...
  alias Crucible.{Attempt, Delegation, Events, Keys}
  ...
  defp release(caller, chain, child_id) do
    with %{parent_execution_id: parent} <-
  ...
  defp admit(caller, chain, child) do
    with {:ok, input} <-
      ...
      Crucible.admit_child(chain.authority, child.reference, child.need, input,
        ctx: chain.ctx,
        parent_execution_id: caller.execution_id,
        root_execution_id: chain.root_execution_id,
        attempt: caller.attempt,
  ...
  defp answer_child(caller, claimed) do
    with {:ok, keys} <- Keys.attempt_keys(Map.take(caller, @attempt_fields)),
      ...
      {:error, reason} ->
        Logger.error(
          "[Crucible.Host.Children] a child of #{caller.execution_id} was admitted but " <>
