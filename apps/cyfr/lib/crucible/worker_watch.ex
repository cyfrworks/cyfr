...
defmodule Crucible.WorkerWatch do
  @moduledoc """
  ...
  *write*: the `job_claims` row `(kind: "worker_watch", key: <service id>)`
  ...
  (`Crucible.Lapse.boot/3`), and the attempt process open for each
  is stopped without closing its run
  (`Crucible.Attempt.stop_unclosed/2`), so its waiter answers the
  ...
  (`Crucible.Attempt`); the lapse narrows them to the ones
  ...
  `Crucible.Dispatch` (`fresh_boot/2`): a start addressed to it
  ...
  alias Crucible.{Attempt, Lapse, WorkerClient}
  ...
  def handle_info({:DOWN, _ref, :process, poller, reason}, state) do
    case Map.pop(state.polls, poller) do
      ...
      {id, polls} ->
        Logger.error("[Crucible.WorkerWatch] the poll of #{id} crashed: #{inspect(reason)}")
  ...
  defp lapse_boot(id, boot, reported, why) do
    case Lapse.boot(id, boot, Enum.uniq(reported ++ open_attempts())) do
      {:ok, lapsed} ->
        ...
        Logger.warning(
          "[Crucible.WorkerWatch] #{id}: boot #{boot} #{why}; " <>
      ...
      {:error, :unavailable} ->
        Logger.error(
          "[Crucible.WorkerWatch] #{id}: boot #{boot} #{why}; its running attempts " <>
            "could not be lapsed"
        )
  ...
  # The attempts open on this member: `Crucible.Attempt` registers
