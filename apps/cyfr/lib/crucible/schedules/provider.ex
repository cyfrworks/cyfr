...
defmodule Crucible.Schedules.Provider do
  @moduledoc """
  ...
  (`Crucible.Schedules.Scheduler`) what changed. A schedule's `concurrency`
  ...
  def handle("schedule", %Context{} = ctx, %{"action" => "create"} = args) do
    with :ok <- validate_required(args, ["name", "cron_expression", "reference"]),
      ...
      case Arca.CronSchedule.create(attrs) do
        {:ok, schedule} ->
          Crucible.Schedules.Scheduler.add(schedule.id)
    ...
    with {:schedule, {:ok, schedule}} <-
      ...
      with {:ok, update_attrs} <- maybe_resolve_reference(ctx, args, update_attrs),
        ...
        case Arca.CronSchedule.update(Sanctum.Context.actor(ctx), schedule.id, update_attrs) do
          {:ok, updated} ->
            Crucible.Schedules.Scheduler.update(updated.id)
    ...
    case Arca.CronSchedule.get_by_id_or_name(Sanctum.Context.actor(ctx), id) do
      ...
      {:ok, schedule} ->
        case Arca.CronSchedule.update(Sanctum.Context.actor(ctx), schedule.id, %{status: "paused"}) do
          {:ok, updated} ->
            Crucible.Schedules.Scheduler.pause(updated.id)
            {:ok, format_schedule(updated)}
    ...
    case Arca.CronSchedule.get_by_id_or_name(Sanctum.Context.actor(ctx), id) do
      ...
      {:ok, schedule} ->
        ...
        case Arca.CronSchedule.update(Sanctum.Context.actor(ctx), schedule.id, %{
          ...
          {:ok, updated} ->
            Crucible.Schedules.Scheduler.resume(updated.id)
            {:ok, format_schedule(updated)}
    ...
    case Arca.CronSchedule.get_by_id_or_name(Sanctum.Context.actor(ctx), id) do
      ...
      {:ok, schedule} ->
        case Arca.CronSchedule.soft_delete(Sanctum.Context.actor(ctx), schedule.id) do
          {:ok, _} ->
            Crucible.Schedules.Scheduler.remove(schedule.id)
  ...
  defp validate_cron(expr) do
    case Crucible.Schedules.Cron.parse(expr) do
      {:ok, _} ->
        case Crucible.Schedules.Cron.min_interval_seconds(expr) do
  ...
  defp compute_next_run(cron_expression) do
    case Crucible.Schedules.Cron.parse(cron_expression) do
      {:ok, parsed} -> Crucible.Schedules.Cron.next_run(parsed, DateTime.utc_now())
      error -> error
    end
  end
  ...
  # (`Crucible.Schedules.Scheduler` skips the run).
  ...
  defp decode_stored(json, default, field) when is_binary(json) do
    case Prima.Json.decode(json) do
      ...
      {:error, :invalid_json} ->
        Logger.warning(
          "[Crucible.Schedules.Provider] stored #{field} is not valid JSON (#{byte_size(json)} bytes)"
        )
