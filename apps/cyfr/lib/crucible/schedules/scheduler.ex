...
defmodule Crucible.Schedules.Scheduler do
  @moduledoc """
  ...
  and running it as a root (`Crucible.run_root/5`).
  ...
  defp start_occurrence(schedule, occurrence, ctx, exec_reference, input, state, generation) do
    ...
    case Task.Supervisor.start_child(Crucible.Schedules.TaskSupervisor, fn ->
  ...
  defp run(schedule, occurrence, ctx, exec_reference, input, generation) do
    ...
    run_result =
      Crucible.run_root(ctx, {:id, schedule.profile_id}, exec_reference, input,
        execution_id: execution_id,
        occurrence_id: occurrence.id,
        schedule_id: schedule.id,
        class: :background,
  ...
  defp stored_metadata(json) when is_binary(json) do
    case Prima.Json.decode(json) do
      ...
      {:error, :invalid_json} ->
        Logger.warning(
          "[Crucible.Schedules.Scheduler] stored metadata is not valid JSON (#{byte_size(json)} bytes)"
        )
