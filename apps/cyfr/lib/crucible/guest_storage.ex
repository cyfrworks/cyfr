...
defmodule Crucible.GuestStorage do
  @moduledoc """
  ...
  the `storage` host call (`Crucible.Host.Storage`). A runner parses
  ...
  defp caught(scope, op, path, content) do
    ...
    exception ->
      Logger.error(
        "[Crucible.GuestStorage] #{op} raised: " <>
  ...
  defp public_quota(op, path, content, %{quota: %{} = quota, ctx: ctx}) when op in @writing do
    ...
    case scope_usage(ctx, path) do
      ...
      _unreadable ->
        Logger.warning(
          "[Crucible.GuestStorage] public-quota usage unreadable for " <>
  ...
  defp uncertain(verb, path, reason) do
    Logger.error("[Crucible.GuestStorage] a #{verb} is uncertain: #{reason}")
  ...
  defp store_fault(what, reason) do
    Logger.error("[Crucible.GuestStorage] #{what} failed: #{inspect(reason)}")
