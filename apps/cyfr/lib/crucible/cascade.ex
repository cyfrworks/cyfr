...
defmodule Crucible.Cascade do
  ...
  each is stopped (`Crucible.Dispatch.stop/2`), so neither its waiter
  ...
  alias Crucible.{Dispatch, Events, Record, Telemetry}
  ...
  def fail_children_of(execution_id) do
    case Arca.Execution.list_running_children(execution_id) do
      ...
      {:error, reason} = error ->
        ...
        Logger.error(
          "[Crucible.Cascade] children of #{execution_id} could not be listed: " <>
            inspect(reason)
        )
