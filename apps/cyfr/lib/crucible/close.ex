...
defmodule Crucible.Close do
  ...
  `Crucible.Attempt`, which calls these functions from its own
  process. A refusal before the attempt opens (`Crucible.Admission`)
  ...
  alias Crucible.{Cascade, Events, Record, StepSpans, Telemetry}
  ...
  def complete(%__MODULE__{} = close, secrets, output, exec_metadata) do
    ...
    with :ok <- check_application_error(close, secrets, masked_output),
      ...
      if write_result == {:error, :not_running} do
        Logger.info(
          "[Crucible.Close] execution #{completed_record.id} finished after cancel; " <>
  ...
  def exception_message(exception, stacktrace) do
    Logger.error(
      "[Crucible.Close] execution raised: " <>
  ...
  defp audit_error(record, {:error, reason}) do
    Logger.error(
      "[Crucible.Close] Failed to write completed record #{record.id}: #{inspect(reason)}. " <>
    ...
    case Record.write_started(record, close.admission) do
      ...
      {:error, reason} ->
        if Arca.Execution.barrier_refusal?(reason) do
          ...
          Logger.error(
            "[Crucible.Close] Failed to write started record #{record.id}: " <>
  ...
  defp ended_failed(failed_record, written, message, admitted \\ :admitted) do
    case written do
      ...
      {:error, :not_running} when admitted == :unadmitted ->
        Logger.error(
          "[Crucible.Close] execution #{failed_record.id} was never admitted; " <>
            "its failure has no row to close"
        )

      {:error, :not_running} ->
        Logger.info(
          "[Crucible.Close] execution #{failed_record.id} failed after cancel; " <>
            "the cancelled row stands"
        )

      {:error, reason} ->
        Logger.error(
          "[Crucible.Close] Failed to write failed record #{failed_record.id}: " <>
