...
defmodule Aqua.Standing do
  ...
  `:once` answers this call alone. `:conversation` and `:always` are
  ...
  `false` takes no standing answer at all, `"conversation"` none at agent
  ...
  def check(intent, scope) when scope in [:conversation, :always] do
    ...
    cond do
      ...
      standing == :conversation and scope == :always ->
  ...
  runs: none for `:once`, an allow at conversation or agent scope, a deny
  ...
  def rows(%Context{} = ctx, turn, %{"tool" => tool, "action" => action}, scope)
      when scope in [:conversation, :always, :never] and is_binary(tool) and is_binary(action) do
    {grant_scope, effect} =
      case scope do
        :conversation -> {"conversation", "allow"}
        :always -> {"agent", "allow"}
        :never -> {"agent", "deny"}
      end
  ...
  def answer(:conversation), do: "\"Always for this conversation\""
