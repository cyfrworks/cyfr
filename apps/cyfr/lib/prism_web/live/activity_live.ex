defmodule PrismWeb.ActivityLive do
  @moduledoc """
  ...
  def handle_event("time_filter", %{"time" => time}, socket) do
    {:noreply,
     socket
     |> assign(:time_filter, normalize_filter(time))
     |> fetch_logs()}
  end
  ...
  # Computed here (one correlate-style query batched into list iteration) so the
  # row badge doesn't require expanding every row up front. For now we issue
  # one stats query per visible row; at @page_size = 50 that's tolerable, and
  # callers concerned about perf can switch to a single GROUP BY query later.
  defp build_fan_outs(socket, logs) do
    logs
    |> Enum.reduce(%{}, fn log, acc ->
      id = log[:id] || log["id"]

      case id && call_tool(socket, "mcp_log", %{"action" => "correlate", "request_id" => id}) do
        {:ok, %{executions: execs}} when is_list(execs) ->
          Map.put(acc, id, length(execs))
