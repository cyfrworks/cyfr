...
defmodule Emissary.External.Reconciler do
  @moduledoc """
  ...
  (`Emissary.External.ServerSupervisor.live/0`), rereads the revision
  tokens of the entries it resolved against
  (`Emissary.External.Server.vault_revisions/2`,
  ...
  def handle_info(%AthanorArchived{athanor_id: athanor_id}, state) do
    Emissary.External.ServerSupervisor.stop_athanor(athanor_id)
    ...
    case while_held(fn -> reconcile(athanor_id, entry_id, meta) end) do
      ...
      {:error, reason} ->
        ...
        if attempt_no < @max_fast_retries do
          ...
          Logger.warning(
            "[Emissary.External.Reconciler] reconcile of #{entry_id} still failing after " <>
  ...
  defp reconcile(athanor_id, entry_id, meta) do
    ...
    _released = Emissary.External.Backends.release_referencing(athanor_id, names)
  ...
  defp stop_servers(affected, athanor_id, entry_id, ctx) do
    results =
      Enum.map(affected, fn server ->
        Logger.info(
          "[Emissary.External.Reconciler] restarting '#{server.name}' — " <>
  ...
  def sweep_revisions do
    Emissary.External.ServerSupervisor.live()
    |> Enum.flat_map(fn {name, athanor_id, pid} ->
      case Emissary.External.Server.vault_revisions(pid) do
  ...
  defp sweep_athanor(athanor_id, held) do
    ...
    with {:ok, current} <- Sanctum.VaultReader.revisions(athanor_id, Enum.uniq(names)),
      ...
      _released = Emissary.External.Backends.release_referencing(athanor_id, moved_names)
  ...
  defp references?(server, names) do
    ...
    referenced = headers ++ Emissary.External.BackendDefinition.entry_names(config["backends"])
