...
defmodule Emissary.External.Backends do
  @moduledoc """
  ...
  `Emissary.External.Server`. The bridge runs an owner's backends at a
  ...
  alias Emissary.External.StatusRedaction
  ...
  defp stand_down(state, {_athanor, server} = key) do
    Logger.warning(
      "[Emissary.External.Backends] the claim on #{server} is held elsewhere; standing down without releasing it"
    )
  ...
  defp check_generation(state, generation) do
    if generation == state.generation do
      ...
      Logger.warning(
        "[Emissary.External.Backends] the control-plane generation changed; greeting the bridge"
      )
  ...
  defp observe_boot(state, _other) do
    Logger.warning(
      "[Emissary.External.Backends] the bridge restarted; greeting it and syncing live owners"
    )
  ...
  defp post(spec, control_key, body_map) do
    ...
    with :ok <- within_control_size(body),
      ...
      case Sanctum.Egress.pinned_request(:post, spec.url <> "/control", headers, body, opts) do
  ...
  defp settle_job(state, {:hello}, spec, {:http, 200, _boot, %{"boot" => boot} = body})
    ...
    Logger.info(
      "[Emissary.External.Backends] greeted bridge #{boot} under generation #{spec.generation}"
    )
  ...
  defp settle_job(state, {:reconcile}, _spec, {:http, 200, _boot, body}) do
    case body["released"] do
      [_ | _] = released ->
        Logger.info("[Emissary.External.Backends] reconcile released #{length(released)} owners")
  ...
  defp lapsed(state, %{"athanor" => athanor, "server" => server, "e" => epoch}) do
    ...
    case state.owners[key] do
      %{epoch: ^epoch, live?: true} ->
        Logger.warning(
          "[Emissary.External.Backends] the bridge no longer runs #{server}; its epoch was raised"
        )
  ...
  defp log_failure(what, result) do
    case refusal(result) do
      ...
      :claimed_elsewhere ->
        Logger.info(
          "[Emissary.External.Backends] #{what} refused: another member of the cell holds the backend"
        )

      :bridge_refused_signature ->
        Logger.error(
          "[Emissary.External.Backends] the bridge refused the #{what} signature — CYFR_MCP_BRIDGE_KEY must " <>
            "be the same value for cyfr and the bridge"
        )

      reason ->
        Logger.warning("[Emissary.External.Backends] #{what} failed: #{inspect(reason)}")
    end
  end
end
