...
defmodule Emissary.External.Servers do
  @moduledoc """
  ...
  `Emissary.External.Server` needs resolved headers, a timeout and the
  ...
  (`Emissary.External.Provider`) builds a config to create, test or refresh
  a server, and `Emissary.External.Proxy` builds one to discover and
  ...
  inside `Emissary.External.Server.resolve_headers/2`, at the moment of
  ...
  @doc """
  The keyword config `Emissary.External.ServerSupervisor.ensure_started/1`
  ...
  def ensure_started(server, %Context{} = ctx) do
    case Emissary.External.ServerSupervisor.ensure_started(server_config(server, ctx)) do
      {:ok, _pid} -> Emissary.External.Server.get_tools(server.name, ctx.athanor_id)
