...
defmodule Emissary.External.ServerSupervisor do
  @moduledoc """
  DynamicSupervisor for external MCP server connections.

  Manages `Emissary.External.Server` GenServer processes. Servers are started
  ...
  def ensure_started(config) do
    name = config[:name]
    athanor_id = Emissary.External.Server.athanor_id!(config)
    digest = config_digest(config)

    case Registry.lookup(Emissary.External.ServerRegistry, {name, athanor_id}) do
      [{pid, ^digest}] ->
        {:ok, pid}
  ...
  defp start_child(config) do
    case DynamicSupervisor.start_child(
           __MODULE__,
           {Emissary.External.Server, config}
         ) do
      {:ok, pid} -> {:ok, pid}
  ...
  def live do
    Registry.select(Emissary.External.ServerRegistry, [
      {{{:"$1", :"$2"}, :"$3", :_}, [], [{{:"$1", :"$2", :"$3"}}]}
    ])
  end
