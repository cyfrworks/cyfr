...
defmodule Crucible.AttemptMaskingTest do
  @moduledoc """
  ...
  (`Crucible.Host`). The vault field begins with the token, so text
  ...
  test "a dispensed token and the vault field are masked in the events, output, row, payload and answers",
    ...
    refute_unmasked(Crucible.Events.since(id, {0, 0}, ctx.athanor_id))
  ...
  test "a crash report shows neither a credential, the held text nor the call that crashed it",
    ...
    log =
      capture_log(fn ->
        assert {:error, :lost} =
                 Crucible.Attempt.call(fixture.execution_id, caller, {:push_deltas, @field})

        assert_receive {:DOWN, ^ref, :process, _, _}, 5_000
        Process.sleep(100)
      end)

    assert log =~ "Crucible.Attempt"
    refute_unmasked(log)
  end
