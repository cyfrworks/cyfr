...
defmodule Crucible.RunTest do
  @moduledoc """
  A root run through `Crucible` on the opus worker service, and the
  ...
  defp run_consented(ctx, input) do
    Crucible.run_root(ctx, :default, @test_ref, input)
  end
  ...
  describe "run_root/5" do
    ...
    test "refuses an unconsented component instead of guessing", %{ctx: ctx} do
      assert {:error, :no_profile} =
               Crucible.run_root(ctx, :default, "reagent:local.nonexistent:0.1.0", %{})
    end

    test "returns error for empty reference", %{ctx: ctx} do
      assert {:error, {:invalid_reference, _reason}} =
               Crucible.run_root(ctx, :default, "", %{})
    end
  end
  ...
  describe "get/2" do
    test "returns :not_found for non-existent execution", %{ctx: ctx} do
      assert {:error, :not_found} = Crucible.get(ctx, "exec_nonexistent")
    end
    ...
    test "retrieves execution after run", %{ctx: ctx} do
      ...
      assert records != []
      record = hd(records)
      {:ok, fetched} = Crucible.get(ctx, record.id)
  ...
  describe "cancel/2" do
    test "returns :not_found for non-existent execution", %{ctx: ctx} do
      assert {:error, :not_found} = Crucible.cancel(ctx, "exec_nonexistent")
    end
    ...
    test "returns :not_cancellable for failed execution", %{ctx: ctx} do
      ...
      {:ok, records} = Crucible.list(ctx)
      record = hd(records)
      assert {:error, :not_cancellable} = Crucible.cancel(ctx, record.id)
    end
  end
end
