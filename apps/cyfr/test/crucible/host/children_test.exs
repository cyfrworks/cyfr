...
defmodule Crucible.Host.ChildrenTest do
  @moduledoc """
  ...
  describe "admit_child" do
    test "a spawned child is claimed for the calling runner and holds its slots and charge until its terminal write",
      ...
      assert [{^pid, {:dispatched, ^endpoint}}] =
               Registry.lookup(Crucible.Registry, child.execution_id)
      ...
      assert Prima.Slots.status(Crucible.Slots).child_active == children_before + 1
      ...
      assert Prima.Slots.status(Crucible.Slots).child_active == children_before
  ...
  describe "a formula that is ending" do
    test "once cancelled, refuses a synchronous tool call and stops its attempt", %{ctx: ctx} do
      ...
      assert {:ok, %{cancelled: true}} = Crucible.cancel(ctx, fixture.execution_id)
