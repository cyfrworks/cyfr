...
defmodule Arca.PublicationContractTest.Double do
  @moduledoc false
  # The adapter the contract is first proven on: Local's bytes, the
  # conditional writes and the prefix listing with real preconditions,
  # and no tree swap — an object store's shape.
  use Arca.Storage.TestDouble
end

defmodule Arca.PublicationContractTest.Faults do
  @moduledoc false
  ...
  defp optional(callback, args, unsupported) do
    adapter = base()
  ...
  def replace_tree(ctx, path, files) do
    through(:replace_tree, path, fn ->
      optional(:replace_tree, [ctx, path, files], {:error, :atomic_replace_unsupported})
    end)
  end

  @impl true
  def put_if_none_match(ctx, path, content) do
    through(:put_if_none_match, path, fn ->
      optional(:put_if_none_match, [ctx, path, content], {:error, :unsupported})
    end)
  end

  @impl true
  def put_if_match(ctx, path, content, precondition),
    do: optional(:put_if_match, [ctx, path, content, precondition], {:error, :unsupported})

  @impl true
  def list_prefix(ctx, prefix), do: optional(:list_prefix, [ctx, prefix], {:error, :unsupported})
end

defmodule Arca.PublicationContractTest do
  @moduledoc """
  The publication contract of a unit, on every adapter it is proven on: a
  database pointer names a committed immutable revision and a journal
  records the same commit; a complete object set is not proof of
  publication; repair never promotes a losing writer's objects.

  Every case drives `Arca.Overlay` over the adapter under test, with
  faults injected between the two, so the contract is the same whatever
  stores the bytes. An adapter joins the suite as one more entry of
  `@adapters`.
  """

  # The double is an object store's shape: conditional writes, a prefix
  # listing, no tree swap. Local swaps the served tree.
  @adapters [
    %{adapter: Arca.PublicationContractTest.Double},
    %{adapter: Arca.Adapters.Local}
  ]

  use ExUnit.Case, async: false, parameterize: @adapters

  alias Arca.PublicationContractTest.Faults
  ...
  describe "a commit" do
    test "publishes the revision: the pointer, one journal row, the objects served, nothing staged",
      ...
      assert [%{prior_revision: nil, new_revision: ^revision, content_identity: "sha256:" <> _}] =
               journal(actor, unit)
  ...
  describe "a move to the served location that fails" do
    ...
    test "is never a lost commit, and a repair that fails part-way is repaired again", %{
      ...
      fail_serving = fn ->
        Faults.inject(fn op, path ->
          cond do
            op == :replace_tree and served?(path, unit) -> {:error, :enospc}
            op == :put and served?(path, unit) and List.last(path) == "b.txt" -> {:error, :enospc}
            true -> :pass
          end
        end)
      end
  ...
  describe "a complete object set no row names" do
    test "is not a published unit to any reader", %{ctx: ctx, actor: actor, unit: unit} do
      ...
      assert {:error, :exists} =
               Arca.Overlay.commit_unit(ctx, unit, revision("one"), cap: :exempt, if_absent: true)
    ...
    test "a subtree replacement carries served objects into the unit's first revision", %{
      ...
      assert :ok =
               Arca.Overlay.replace_subtree(ctx, unit, ["sub"], [{["c.txt"], "c"}], cap: :exempt)
      ...
      assert {:error, :not_found} =
               Arca.Overlay.replace_subtree(ctx, bare, ["sub"], [{["c.txt"], "c"}], cap: :exempt)
