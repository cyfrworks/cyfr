...
defmodule Arca.MembersTest do
  ...
  describe "the actor is the first argument, and a wrong one refuses before any query" do
    ...
    test "an actor whose athanor is the empty string is refused, not answered emptily" do
      watch_queries!()
      # `""` is an identity that was never resolved, the same thing
      # `Sanctum.Context.build/1` refuses outright. Taking it would filter
      # on `athanor_id == ""`, match nothing, and answer an empty roster
    ...
    test "a context, or a bare athanor id, raises before any query" do
      watch_queries!()
      athanor = group!()
      drain_queries!()

      ctx = %Sanctum.Context{athanor_id: athanor.id, scope: :athanor}
      refused!(:seat, [ctx, %{user_id: "usr_1"}])
      refused!(:list, [ctx, []])
      refused!(:count_active, [ctx])
