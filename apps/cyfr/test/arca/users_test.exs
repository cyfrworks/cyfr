...
defmodule Arca.UsersTest do
  ...
  describe "the actor is the first argument, and a wrong one refuses before any query" do
    ...
    test "a context, or a bare person id, raises before any query" do
      watch_queries!()
      user = person!()
      drain_queries!()

      ctx = %Sanctum.Context{user_id: user.id, scope: :platform}
      refused!(:get, [ctx, user.id])
      refused!(:get_by_identity, [ctx, "github|iss|sub"])
      refused!(:list, [ctx, []])
