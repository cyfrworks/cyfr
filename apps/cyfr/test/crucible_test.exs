...
defmodule CrucibleTest do
  ...
  defp run(ctx) do
    Crucible.run_child(AuthorityFixtures.root!(), "reagent:local.off-graph:1.0.0", nil, %{},
      ctx: ctx
    )
  end
  ...
  test "with no worker service configured, execution is unavailable and a run is refused", %{
    ...
    assert Crucible.events_since("exec_1", {0, 0}, ctx.athanor_id) == []
  end
