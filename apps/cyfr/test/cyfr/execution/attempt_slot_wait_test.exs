...
defmodule Cyfr.Execution.AttemptSlotWaitTest do
  ...
  `Cyfr.Execution.Slots` held by the test, so a run is queued until the
  test frees one. A cancel is `Cyfr.Execution.cancel/2`. What a queued task
  ...
  describe "a background root queued under its athanor's full share" do
    ...
    test "cancelled, it frees its place: the next waiter of the athanor takes the share freed next",
      ...
      assert {:ok, %{cancelled: true}} = Cyfr.Execution.cancel(ctx, first)
  ...
  defp spawn_within(ctx, authority, root, id, ms) do
    Cyfr.Execution.run_child(authority, @ref, nil, input(),
      ctx: ctx,
      execution_id: id,
