...
defmodule Emissary.MCP.ChainLoggingTest do
  ...
  test "an in-chain call gets its own row, filed under the request that started it" do
  ...
  test "a call the transport already logged is not logged twice" do
    request_id = Emissary.UUID7.request_id()
    ctx = %{Sanctum.TestContext.local() | request_id: request_id}
  ...
  test "an internal caller with no request id becomes its own root" do
    ctx = %{Sanctum.TestContext.local() | request_id: nil}
  ...
  test "mcp_log never logs itself" do
    ctx = %{Sanctum.TestContext.local() | request_id: nil}
