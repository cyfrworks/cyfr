...
defmodule Sanctum.Auth.OAuthTest do
  ...
  describe "authenticate/1 with allowed_user restriction" do
    setup do
      ...
      on_exit(fn ->
        Application.delete_env(:ueberauth, Ueberauth.Strategy.Github.OAuth)
        Application.delete_env(:cyfr, :allowed_users)
      end)
    ...
    test "allows user when email matches allowed_users" do
      Application.put_env(:cyfr, :allowed_users, ["alice@example.com", "bob@example.com"])
    ...
    test "rejects user when email not in allowed_users" do
      Application.put_env(:cyfr, :allowed_users, ["alice@example.com"])
      ...
      {:error, :user_not_allowed} = OAuth.authenticate(params)
    end
    ...
    test "allows any user when allowed_users is nil" do
      ...
      {:ok, _user} = OAuth.authenticate(params)
    end

    test "allows any user when allowed_users is empty list" do
      Application.put_env(:cyfr, :allowed_users, [])
