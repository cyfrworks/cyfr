defmodule Sanctum.ContextJWTTest do
  ...
  setup do
    ...
    original_key = Application.get_env(:cyfr, :jwt_signing_key)
    Application.put_env(:cyfr, :jwt_signing_key, @test_key)

    on_exit(fn ->
      if original_key do
        Application.put_env(:cyfr, :jwt_signing_key, original_key)
      else
        Application.delete_env(:cyfr, :jwt_signing_key)
      end
    end)
  ...
  describe "from_jwt/1" do
    ...
    test "defaults to project scope" do
      jwt = create_jwt(%{"sub" => "user_123"})
    ...
    test "rejects unknown scope string" do
      jwt = create_jwt(%{"sub" => "user_123", "scope" => "invalid"})
      assert {:error, {:invalid_scope, "invalid"}} = Context.from_jwt(jwt)
    end
  end
  ...
  describe "JWT expiration validation" do
    ...
    test "respects custom clock skew configuration" do
      ...
      original_skew = Application.get_env(:cyfr, :jwt_clock_skew_seconds)
      Application.put_env(:cyfr, :jwt_clock_skew_seconds, 300)

      on_exit(fn ->
        if original_skew do
          Application.put_env(:cyfr, :jwt_clock_skew_seconds, original_skew)
        else
          Application.delete_env(:cyfr, :jwt_clock_skew_seconds)
        end
      end)
  ...
  describe "JWT key validation" do
    test "rejects short JWT signing key" do
      ...
      Application.put_env(:cyfr, :jwt_signing_key, short_key)
