...
defmodule Sanctum.ProviderTest do
  ...
  describe "resource_templates/0" do
    test "returns no templates" do
      assert Provider.resource_templates() == []
    end
  end
  ...
  describe "session tool" do
    test "whoami returns local user only (registry identity moved to Compendium.Provider.registry.whoami)",
         %{ctx: ctx} do
      ...
      # lives on Compendium.Provider.registry.whoami.
    ...
    test "whoami returns error when not authenticated" do
      ...
      {:error, msg} = Provider.handle("session", ctx, %{"action" => "whoami"})
    ...
    test "logout retires the session that called it", %{ctx: ctx} do
      ...
      {:ok, result} = Provider.handle("session", session_ctx, %{"action" => "logout"})
    ...
    test "logout refuses when an API key authenticated the call", %{ctx: ctx} do
      {:error, msg} = Provider.handle("session", ctx, %{"action" => "logout"})
      assert err_msg(msg) =~ "No session to log out"
    end

    test "invalid action returns error", %{ctx: ctx} do
      {:error, msg} = Provider.handle("session", ctx, %{"action" => "invalid"})
  ...
  describe "secret tool retired" do
    test "the secret tool is no longer routable", %{ctx: ctx} do
      {:error, msg} = Provider.handle("secret", ctx, %{"action" => "list"})
  ...
  describe "tincture_visibility invalid action" do
    test "returns error for unknown action", %{ctx: ctx} do
      {:error, msg} =
        Provider.handle("tincture_visibility", ctx, %{"action" => "delete"})
  ...
  describe "session.device-init / device-poll gate" do
    test "device-init is available when auth_provider is OAuth", %{ctx: ctx} do
      ...
      try do
        ...
        result =
          Provider.handle("session", ctx, %{"action" => "device_init", "provider" => "github"})
