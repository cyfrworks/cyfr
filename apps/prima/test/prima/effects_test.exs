...
defmodule Prima.EffectsTest do
  @moduledoc """
  ...
  @roster %{
    Prima.Boot => {:primitive, [:persistent_term]},
    Prima.RateLimiter => {:primitive, [:ets, :logger, :otp, :process, :system]},
    Prima.LoggerContext => {:primitive, [:logger]},
    Prima.JsonFormatter => {:primitive, [:app_env, :persistent_term]},
    Prima.Slots => {:primitive, [:logger, :otp, :process, :system]},
    Prima.Caps => {:primitive, [:code, :persistent_term]},
    ...
    Prima.Provider => {:code_probe, [:code]}
  }
  ...
  @wall_clock_pins [{Prima.UUID7, {:generate, 0}}]

  @id_entropy_pins [{Prima.UUID7, {:generate_at, 1}}, {Prima.Hex, {:short, 0}}]
  ...
  @id_minters [{Prima.Authority.Budget, {:new, 2}}, {Prima.Boot, {:mint, 0}}]

  @id_functions [
    {Prima.UUID7, :generate},
    {Prima.UUID7, :generate_at},
    {Prima.UUID7, :generate_id},
    {Prima.UUID7, :request_id},
    {Prima.UUID7, :execution_id},
    {Prima.UUID7, :build_id},
    {Prima.Hex, :short}
  ]
  ...
  describe "the compiled view" do
    ...
    test "a removed roster row fails", %{modules: modules} do
      for module <- [Prima.UUID7, Prima.Slots, Prima.WorkerAuth] do
    ...
    test "a stale roster row fails", %{modules: modules} do
      roster = Map.put(@roster, Prima.Digest, {:id, [:entropy]})
      assert {Prima.Digest, [], [:entropy]} in beam_violations(modules, roster)
    ...
    test "Logger is reached only by Slots, RateLimiter and LoggerContext", %{modules: modules} do
      assert reaching(modules, :logger) == [Prima.LoggerContext, Prima.RateLimiter, Prima.Slots]
    end

    test "the wall clock is read only by the identifier generator", %{modules: modules} do
      assert reaching(modules, :wall_clock) == [Prima.UUID7]
    end
    ...
    test "no beam reads Mix, an application spec or configuration at run time",
      ...
      assert reaching(modules, :app_env) == [Prima.JsonFormatter]
    end
  ...
  describe "the roster" do
    ...
    test "the scan is not empty", %{modules: modules, findings: findings} do
      ...
      assert Prima.UUID7 in modules and Prima.WIT in modules
