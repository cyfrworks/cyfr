...
defmodule Prima.LoggerContextTest do
  ...
  describe "unexpected/3" do
    defp line(message, level \\ :warning) do
      ...
      [line] = Regex.run(~r/\[Prima\.LoggerContextTest\] unexpected message: .*/, log)
      line
    end

    defp shape(message),
      do:
        String.replace_prefix(line(message), "[Prima.LoggerContextTest] unexpected message: ", "")
