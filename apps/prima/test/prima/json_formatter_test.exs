...
defmodule Prima.JsonFormatterTest do
  ...
  describe "format/4" do
    ...
    test "metadata values with no String.Chars implementation do not crash the handler" do
      ...
      on_exit(fn ->
        # Standalone, the application configures no formatter: restoring
        # means removing the key, since a nil one breaks every later capture.
        if original,
          do: Application.put_env(:logger, :default_formatter, original),
          else: Application.delete_env(:logger, :default_formatter)

        :persistent_term.erase({Prima.JsonFormatter, :metadata_keys})
      end)

      Application.put_env(:logger, :default_formatter,
        format: {Prima.JsonFormatter, :format},
