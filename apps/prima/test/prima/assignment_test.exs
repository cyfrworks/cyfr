...
defmodule Prima.AssignmentTest do
  @moduledoc """
  ...
  authority `Prima.Authority.from_wire/1` refuses, a component reference
      ...
      Map.merge(
        %{
          ...
          component: %{
            ...
            type: "catalyst",
            digest: Prima.Digest.sha256("artifact"),
            declared_needs: ["source"],
            activation_digest: Prima.Digest.sha256("activation")
          },
          input_digest: Prima.Digest.sha256(~s({"query":"select 1"})),
          timeout_ms: 30_000,
          deadline: @now + 60_000,
  ...
  describe "sign and verify" do
      ...
      root =
        assignment(zero, %{
          ...
          component: %{
            ...
            type: "reagent",
            digest: Prima.Digest.sha256("reagent"),
            declared_needs: [],
    ...
    test "the encoding is canonical: the same assignment signs to the same token",
      ...
      assert {:ok, bytes} == Prima.JCS.encode(Jason.decode!(bytes))
  ...
  describe "a forged token" do
    ...
    test "is refused when its payload was changed", %{assign_key: key, authority: authority} do
      ...
      {:ok, widened} =
        token |> payload() |> Jason.decode!() |> Map.put("fence", 2) |> Prima.JCS.encode()
