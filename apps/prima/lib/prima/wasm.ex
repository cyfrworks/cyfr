...
defmodule Prima.Wasm do
  @moduledoc """
  WASM binary validation for the Compendium registry.
  ...
  the binary actually exports (`Prima.WIT.type_for_export/1`) —
  ...
  ## Usage
      ...
      {:ok, result} = Prima.Wasm.validate(wasm_bytes)
      # => %{
      #      valid: true,
      ...
      {:error, reason} = Prima.Wasm.validate(invalid_bytes)
  ...
  defp check_declared_world(%{format: :component} = validation, type) do
    case Prima.WIT.expected_exports(type) do
      [] ->
  ...
  defp component_suggested_type(exports) do
    exports
    |> Enum.find_value(&Prima.WIT.type_for_export/1)
    |> case do
      nil -> :reagent
  ...
  """
  defdelegate compute_digest(bytes), to: Prima.Digest, as: :sha256
