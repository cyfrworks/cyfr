...
defmodule Prima.WIT do
  ...
  wit_entries =
    for type <- Prima.ComponentRef.executable_types() do
      ...
      if files == [] do
        raise "Prima.WIT: no WIT files found under #{type_dir} — " <>
  ...
  @expected_exports Map.new(@wit_files, fn {type, files} ->
                      world =
                        case List.keyfind(files, ["world.wit"], 0) do
                          ...
                          nil ->
                            raise "Prima.WIT: wit/#{type}/world.wit is missing — " <>
                                    "every executable type declares its world there"
                        end

                      [_, ns, pkg, ver] =
                        Regex.run(~r/^package\s+([a-z0-9-]+):([a-z0-9-]+)@([0-9.]+);/m, world) ||
                          raise("Prima.WIT: wit/#{type}/world.wit has no package line")
                      ...
                      if exports == [] do
                        raise "Prima.WIT: wit/#{type}/world.wit exports nothing — " <>
