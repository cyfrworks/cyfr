...
defmodule Prima.Limits do
  @moduledoc """
  The resolved numeric limits for one node of a consented execution graph.

  Carries exactly the seven fields `Prima.Limits.Ceiling` clamps — and
  ...
  `Prima.Limits.Ceiling.clamped_fields/0` by test so the two cannot drift.
  ...
  ## Examples
      ...
      iex> {:ok, limits} = Prima.Limits.new(%{
      ...>   "timeout" => "30s",
      ...
      iex> Prima.Limits.new(%{"timeout" => "30s"})
  ...
  Locked to `Prima.Limits.Ceiling.clamped_fields/0` by test.
  """
  @spec fields() :: [atom()]
  ...
  # One spelling each of the other shared defaults; `Prima.Authority`'s
  ...
  @default_rate_limit %{requests: 100, window: "1m"}
  ...
  @doc "The default invocation rate limit (100 per minute)."
  @spec default_rate_limit() :: rate_limit()
  def default_rate_limit, do: @default_rate_limit
  ...
  @doc """
  Build a `%Prima.Limits{}` from an atom- or string-keyed map.
  ...
  ## Examples
      ...
      iex> Prima.Limits.parse_duration("30s")
      {:ok, 30_000}

      iex> Prima.Limits.parse_duration("5m")
      {:ok, 300_000}
  ...
  @doc """
  Clamp against a ceiling map. Delegates to `Prima.Limits.Ceiling.clamp/2`
  so there is exactly one clamping implementation.
  """
  @spec clamp(t(), map()) :: t()
  def clamp(%__MODULE__{} = limits, ceiling) when is_map(ceiling) do
    Prima.Limits.Ceiling.clamp(limits, ceiling)
  end
