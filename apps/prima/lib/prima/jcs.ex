...
defmodule Prima.JCS do
  ...
  ## Examples
      ...
      iex> Prima.JCS.encode(%{"b" => 1, "a" => [true, "x"]})
      {:ok, ~s({"a":[true,"x"],"b":1})}

      iex> Prima.JCS.encode(%{"n" => 1.5})
      {:error, {:invalid_value, ["n"], :float_not_permitted}}

      iex> {:ok, digest} = Prima.JCS.hash(%{"a" => 1})
  ...
  `Prima.Digest` — one producer, one spelling.
  """
  @spec hash_binary(binary()) :: String.t()
  defdelegate hash_binary(binary), to: Prima.Digest, as: :sha256
