...
defmodule Prima.WorkerWire do
  ...
  listener and host client are built on it; nothing here holds state.
    ...
    * A request body is the JSON `{"op": <callback>, "args": {...}}`
      ...
      `Prima.WorkerAuth.seal_call/5` seals a host call's body. The route and
    ...
    * An answer is the JSON `{"ok": value}` on success and
      ...
      sealed as `Prima.WorkerAuth.seal_call/5` seals an answer; a worker
  ...
  is not a callback of `behaviour` (`Prima.HostAPI` or `Prima.WorkerAPI`),
  ...
  def read_request_body(behaviour, %{"op" => op, "args" => %{} = args})
      when behaviour in [Prima.HostAPI, Prima.WorkerAPI] and is_binary(op) do
