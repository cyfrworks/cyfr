...
defmodule Prima.Actor do
  ...
  actor matches `%Prima.Actor{athanor_id: id} when is_binary(id)` and
  ...
  The vocabulary is `Prima.TenancyScope`'s, where the identity domain and
  ...
  `%{Prima.Actor.system() | athanor_id: id, scope: :athanor}` keeps the
