...
defmodule Prima.Authority do
  ...
  shared `Prima.Limits` defaults; the other three fields (timeout,
  batch_timeout, max_concurrent_tasks) are deliberately tighter than
  `Prima.Limits.defaults/1`, so they are never derived from it.

  ## Root budget

  `budget` names the root-keyed invoke budget (`Prima.Authority.Budget`):
  ...
  # slot count (`Prima.Slots`, default 16): a parent holds its slot while
  ...
  Options:
    ...
    * `:ceiling` (required) — the ceiling map the blob is clamped to (the
      shape `Prima.Limits.Ceiling.lowered/1` returns); the instance's platform
  ...
  # ============================================================================
  # Child construction (used by Prima.Authority.Transition)
  # ============================================================================
  ...
  # Bounded by the same cap `Prima.Authority.Transition` checks before every
