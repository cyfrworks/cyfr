...
defmodule Prima.HostAPI do
  ...
  (`Prima.WorkerAuth.host_call_header/3`), and its body and answer are
  sealed with the attempt's seal key (`Prima.WorkerAuth.seal_call/5`).
  Before a callback runs, CYFR verifies the header
  (`Prima.WorkerAuth.verify_host_call/5`), refuses a nonce already seen for
  ...
  that worker service's own dispatch key (`Prima.WorkerAuth.report_header/3`)
  and verified with `Prima.WorkerAuth.verify_report/4`, so it speaks only
  ...
  error envelope (`Prima.WitResponse.encode_error/2`), and for a
  `setup_required` refusal its remediation (`Prima.Remediation`).
  """
  @type guest_error ::
  ...
  the wire, `Prima.WorkerAuth.seal_attempt_keys/3`), the input it was
  ...
  CYFR verifies the token (`Prima.Assignment.verify/3`), checks that it names
