...
defmodule Prima.Assignment do
  @moduledoc """
  What CYFR hands a worker to run one execution attempt, and what the
  runner presents to claim it (`c:Prima.HostAPI.attach/2`).

  `sign/2` JCS-encodes an assignment (`Prima.JCS`) and MACs the encoding with
  the assign key (`Prima.WorkerAuth.assign_key/1`). Only CYFR holds that key,
  ...
  ## Fields (v1)
    ...
    * `member` — the boot of the control-plane member that issued it
      (`Prima.Boot.id/0`). It is the one member that holds this attempt's
    ...
    * `actor` — who it runs for (`Prima.Actor`).
    * `authority` — its authority as `Prima.Authority.to_wire/1`'s map, which
      `Prima.Authority.from_wire/1` must accept. The encoding carries no
    ...
    * `component` — its `ref` (a canonical component reference,
      `Prima.ComponentRef`, of its `type`), its `type` (one of
      `Prima.ComponentRef.valid_types/0`), the `digest` of its artifact, the
    ...
    * `input_digest` — `Prima.Digest.sha256/1` of the input bytes, which
  ...
  # only form that verifies is the one `Prima.WorkerWire.base_url/1`
  # answers: a trailing slash or a path is refused rather than trimmed.
  defp read(:base_url, value) do
    if Prima.WorkerWire.base_url(value) == {:ok, value}, do: {:ok, value}, else: :error
  end
  ...
  defp read(:authority, %{} = authority) when not is_struct(authority) do
    case Prima.Authority.from_wire(authority) do
  ...
  defp read(:component, %{"ref" => ref, "declared_needs" => needs} = component)
    ...
    with [] <- Map.keys(component) -- @component_keys,
         true <- Map.get(component, "type") in Prima.ComponentRef.valid_types(),
  ...
  # A reference names its component once, as `Prima.ComponentRef` spells it,
  # and of the type the assignment gives.
  defp canonical_ref?(ref, type) do
    with true <- Regex.match?(@id, ref),
         {:ok, %Prima.ComponentRef{type: ^type} = parsed} <- Prima.ComponentRef.parse(ref) do
      Prima.ComponentRef.to_string(parsed) == ref
    else
      _ -> false
    end
  end
