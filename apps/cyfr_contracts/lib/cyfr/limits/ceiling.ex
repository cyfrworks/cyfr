...
defmodule Cyfr.Limits.Ceiling do
  @moduledoc """
  ...
  `Cyfr.Limits` struct by a ceiling. Reading the configured overrides is the
  ...
  defp lower_of(field, value, hard_max) when field in @duration_fields do
    with {:ok, value_ms} <- Cyfr.Limits.parse_duration(value),
         {:ok, max_ms} <- Cyfr.Limits.parse_duration(hard_max) do
  ...
  `Cyfr.Limits` locks its field set to this list by test. The struct field
  ...
  @doc """
  Clamp a `Cyfr.Limits` struct to a ceiling map. Resource allowlists are
  never clamped — they live on consent edges, not here.
  """
  @spec clamp(Cyfr.Limits.t(), map()) :: Cyfr.Limits.t()
  def clamp(%Cyfr.Limits{} = limits, ceiling) when is_map(ceiling) do
    limits
  ...
  defp clamp_duration_fields(limits, ceiling) do
    Enum.reduce(@duration_fields, limits, fn field, acc ->
      case Map.get(ceiling, field) do
        ...
        max_dur ->
          with {:ok, current_ms} <- Cyfr.Limits.parse_duration(Map.get(acc, field)),
               {:ok, max_ms} <- Cyfr.Limits.parse_duration(max_dur) do
