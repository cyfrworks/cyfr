...
defmodule Cyfr.ComponentRow do
  @moduledoc """
  ...
  one comparator (`Cyfr.Semver`), `inserted_at` as the tiebreak.
  ...
  ## Examples
      ...
      iex> Cyfr.ComponentRow.field(%{name: "widget"}, :name)
      "widget"

      iex> Cyfr.ComponentRow.field(%{"name" => "widget"}, :name)
      "widget"

      iex> Cyfr.ComponentRow.field(%{}, :name)
      nil
  ...
  ## Examples
      ...
      iex> Cyfr.ComponentRow.node_key(%{component_type: "catalyst", publisher: nil, name: "files"})
  ...
  def node_key(row) when is_map(row) do
    Cyfr.ComponentRef.build(
      ...
      Cyfr.ComponentPath.normalize_publisher(field(row, :publisher)),
      field(row, :name)
    )
  end
  ...
  def latest_of(rows) when is_list(rows) do
    rows
    |> Enum.sort(fn a, b ->
      case Cyfr.Semver.compare(a.version, b.version) do
        :gt -> true
        :lt -> false
