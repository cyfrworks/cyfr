...
defmodule Cyfr.EnvValue do
  ...
  (`Cyfr.MacEnvelope.decode_root/1`), decoded. The error names the form,
  never the value.
  """
  ...
  def hex_key(getenv, key) when is_function(getenv, 1) do
    case trimmed(getenv, key) do
      ...
      text ->
        case Cyfr.MacEnvelope.decode_root(text) do
          {:ok, bytes} -> {:ok, bytes}
  ...
  path, query, fragment or credentials (`Cyfr.WorkerWire.base_url/1`),
