...
defmodule Cyfr.CapsTest.Stub do
  @moduledoc false
  @behaviour Cyfr.Caps
...
defmodule Cyfr.CapsTest.Partial do
  @moduledoc false
  def check_storage(_actor, _incoming), do: :ok
end

defmodule Cyfr.CapsTest do
  @moduledoc """
  ...
  alias Cyfr.Actor
  alias Cyfr.Caps
  alias Cyfr.Caps.NotInstalledError
  alias Cyfr.CapsTest.Partial
  alias Cyfr.CapsTest.Stub
  ...
  describe "impl/0 before the boot write" do
    test "raises its named error, saying what is missing" do
      ...
      assert error.message =~ "Cyfr.Caps.install!/1"
  ...
  describe "install!/1" do
    ...
    test "refuses a module that does not answer both questions" do
      ...
      for module <- [Partial, Cyfr.Actor, Cyfr.CapsTest.NoSuchModule] do
        assert_raise ArgumentError, ~r/does not implement Cyfr.Caps/, fn ->
          Caps.install!(module)
        end
      end
