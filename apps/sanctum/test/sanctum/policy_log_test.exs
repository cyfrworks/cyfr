defmodule Sanctum.PolicyLogTest do
  ...
  describe "log/2" do
    test "logs a policy consultation", %{ctx: ctx} do
      result = PolicyLog.log(ctx, %{
        ...
        component_ref: "catalyst:cyfr.stripe-catalyst:1.0.0",
  ...
  describe "get/2" do
    test "retrieves a logged policy consultation", %{ctx: ctx} do
      :ok = PolicyLog.log(ctx, %{
        event_type: "policy_consultation",
        component_ref: "catalyst:cyfr.tool:1.0.0",
      ...
      assert log["component_ref"] == "catalyst:cyfr.tool:1.0.0"
  ...
  describe "list/2" do
    ...
    test "respects limit parameter" do
      ...
      for i <- 1..5 do
        ...
        :ok = PolicyLog.log(ctx, %{
          event_type: "policy_consultation",
          component_ref: "catalyst:local.test:#{i}.0.0",
          host_policy_snapshot: %{},
          decision: "allowed"
        })
      end
    ...
    test "filters by event_type", %{ctx: ctx} do
      ...
      :ok = PolicyLog.log(ctx2, %{
        ...
        component_ref: "catalyst:local.test:2.0.0",
        host_policy_snapshot: %{},
        decision: "denied"
      })
  ...
  describe "log_allowed/4" do
    test "logs an allowed policy consultation", %{ctx: ctx} do
      :ok = PolicyLog.log_allowed(ctx, "catalyst:local.test:1.0.0", %{"timeout" => "30s"},
  ...
  describe "log_denied/5" do
    test "logs a denied policy consultation", %{ctx: ctx} do
      :ok = PolicyLog.log_denied(ctx, "catalyst:local.test:1.0.0", %{}, "Domain not allowed",
        component_type: :catalyst
      )
