...
defmodule Sanctum.Providers.Webhook do
  ...
  def handle(%Context{} = ctx, %{"action" => "list"}) do
    case Sanctum.Webhook.list(ctx) do
      ...
      {:error, reason} ->
        Logger.error("[Sanctum.Providers.Webhook] Failed to list webhooks: #{inspect(reason)}")
  ...
  def handle(%Context{} = ctx, %{"action" => "update", "name" => name} = args) do
    ...
    case Sanctum.Webhook.update(ctx, name, attrs) do
      ...
      {:error, reason} ->
        Logger.error("[Sanctum.Providers.Webhook] Failed to update webhook: #{inspect(reason)}")
  ...
  def handle(%Context{} = ctx, %{"action" => "revoke", "name" => name}) do
    case Sanctum.Webhook.revoke(ctx, name) do
      ...
      {:error, reason} ->
        Logger.error("[Sanctum.Providers.Webhook] Failed to revoke webhook: #{inspect(reason)}")
  ...
  def handle(_ctx, _args) do
    {:error, Prima.Provider.invalid_action("webhook", action_enum())}
  end
