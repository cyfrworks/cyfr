...
// Exit statuses of `cyfr-keeper serve`.
const (
...
// Main runs `cyfr-keeper serve` with the arguments after the subcommand.
func Main(args []string) int {
	log := logx.New("cyfr-keeper")
...
// runRetire runs `cyfr-keeper retire` as the account and returns its exit
...
func (s *server) runRetire(acct Account, paths []string, grace time.Duration) int {
	...
	c, err := s.start(s.self, []string{"cyfr-keeper", "retire"}, helperEnviron(),
