...
defmodule Opus.Test.EdgePolicy do
  @moduledoc """
  Materialize an Authority's current edge + limits as a legacy
  `%Sanctum.Policy{}` — test-side only. The equivalence oracle compares
  this against the legacy resolver until the `policies` drop, and closure
  tests use it to build the emit-cap policy opt. Dies with the struct.
  """
