defmodule Opus.StorageHandlerTest do
  ...
  describe "build_storage_imports/3" do
    test "returns correct namespace structure", %{ctx: ctx, component_ref: ref} do
      policy = %Policy{allowed_storage_paths: ["data/"]}
  ...
  describe "execute/4 - path safety" do
    ...
    test "rejects absolute paths", %{ctx: ctx, component_ref: ref} do
      ...
      assert decoded["error"]["type"] == "storage_path_denied"
      assert decoded["error"]["message"] =~ "Absolute paths"
    end
  ...
  # ============================================================================
  # allowed_storage_paths Enforcement
  # ============================================================================

  describe "execute/4 - allowed_storage_paths enforcement" do
    test "denies when allowed_storage_paths is empty", %{ctx: ctx, component_ref: ref} do
    ...
    test "denies path outside allowed prefixes", %{ctx: ctx, component_ref: ref} do
      policy = %Policy{allowed_storage_paths: ["data/"]}
    ...
    test "supports multiple allowed path prefixes", %{ctx: ctx, component_ref: ref} do
      policy = %Policy{allowed_storage_paths: ["data/", "artifacts/"]}

      :ok = Arca.put(ctx, ["artifacts", "output.json"], "{}")

      request = Jason.encode!(%{"action" => "read", "path" => "artifacts/output.json"})
      result = StorageHandler.execute(request, policy, ctx, ref)
      decoded = Jason.decode!(result)
  ...
  describe "validate_path_safe/1" do
    test "allows normal paths" do
      ...
      assert :ok = StorageHandler.validate_path_safe("a/b/c/d.json")
      assert :ok = StorageHandler.validate_path_safe("file.txt")
    end
    ...
    test "rejects path traversal" do
      ...
      assert {:error, :storage_path_denied, _} = StorageHandler.validate_path_safe("a/b/../../c")
    end
