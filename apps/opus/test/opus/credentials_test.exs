...
defmodule Opus.CredentialsTest do
  ...
  test "the key is read in either case, and an IPv6 bind address parses" do
    ...
    assert {:ok, %{worker_key: @worker_key, bind: {0, 0, 0, 0, 0, 0, 0, 1}}} = Credentials.load(env)
  end
  ...
  test "each value malformed refuses by name" do
    malformed = [
      ...
      host_url: ["cyfr:4300", "ftp://cyfr", "http://", "http://cyfr/host/v1", "http://cyfr?x=1", 1],
  ...
  test "a refusal raises the boot with the value named, never with the value shown" do
    ...
    after
      for key <- Keyword.keys(Application.get_all_env(:opus)), do: Application.delete_env(:opus, key)
