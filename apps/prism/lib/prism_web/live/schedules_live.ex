defmodule PrismWeb.SchedulesLive do
  ...
  def mount(_params, _session, socket) do
    if connected?(socket) do
      Phoenix.PubSub.subscribe(Emissary.PubSub, "schedules")
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select
                  ...
                  class={input_class()}
                >
                  <option value="" disabled selected={@cron_preset == ""}>Select a schedule...</option>
              ...
              <div :if={@cron_preset == "custom"}>
                <label class="block text-xs font-medium text-gray-400 mb-1">Custom Cron Expression</label>
                <input
                  type="text"
                  name="cron_custom"
                  value={@cron_custom}
              ...
              <div>
                ...
                <%= if @components != [] do %>
                  <select
                    ...
                    class={input_class()}
                  >
                    <option value="" disabled selected={@form[:reference].value == ""}>Select a component...</option>
                ...
                <% else %>
                  <input
                    ...
                    class={"#{input_class()} font-mono"}
                  />
                  <p class="mt-1 text-xs text-gray-500">No registered components found. Enter a reference manually.</p>
                <% end %>
              </div>
              <div>
                <label class="block text-xs font-medium text-gray-400 mb-1">Input (JSON, optional)</label>
                <input
                  type="text"
                  name="input"
      ...
      <.card>
        ...
        <div :if={!@loading && @schedules != []} class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-800 table-fixed">
            <thead>
              <tr>
                <th class="w-[16%] px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Name</th>
                <th class="w-[20%] px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Reference</th>
                <th class="w-[14%] px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Schedule</th>
                <th class="w-[10%] px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Status</th>
                <th class="w-[12%] px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Next Run</th>
                <th class="w-[12%] px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Last Run</th>
                <th class="w-[6%] px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Runs</th>
                <th class="w-[10%] px-4 py-3 text-right text-xs font-medium uppercase tracking-wider text-gray-500"></th>
              </tr>
            </thead>
              ...
              <%= for sched <- @schedules do %>
                <tr class="hover:bg-gray-800/50">
                  ...
                  <td class="px-4 py-3 text-sm whitespace-nowrap">
                    ...
                    <span class="block text-xs text-gray-500 font-mono">{f(sched, :cron_expression)}</span>
                  </td>
                  <td class="px-4 py-3 text-sm whitespace-nowrap">
