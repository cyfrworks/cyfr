...
import { useOrchestratorStore, AVAILABLE_TOOLS } from "../../state/orchestrator-store";
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          {orchestrators.map((orch) => (
            <div key={orch.name} className="space-y-2">
              <AgentCard agent={orch} providers={readyProviders} client={client} />
              ...
              {subAgents
                ...
                .map((sub) => (
                  <div key={sub.name} className="ml-4">
                    <AgentCard agent={sub} providers={readyProviders} client={client} />
                  </div>
                ))}
...
function AgentCard({
  ...
  agent: AgentDetail;
  providers: { key: string; label: string; models: string[]; catalystRef: string }[];
  ...
  const currentProvider = agent.catalyst_ref
    ...
    : "";
  const currentModels = providers.find((p) => p.key === currentProvider)?.models ?? [];
  const isInherited = agent.type === "sub-agent" && !agent.catalyst_ref && !agent.model;
  const nativeSearchActive = agent.visible_tools?.includes("native_search") ?? false;
  ...
  const handleToggleTool = (tool: string) => {
    if (!client) return;
    store.getState().toggleTool(client, agent.name, tool);
  };
  ...
  return (
    <div className="rounded-lg border border-border-default bg-surface-raised p-3 space-y-2">
      ...
      {agent.type === "sub-agent" && agent.description && (
        <p className="text-[10px] text-text-muted line-clamp-2">{agent.description}</p>
      )}
      ...
      <div className="flex flex-wrap gap-1">
        {AVAILABLE_TOOLS.map((tool) => {
          const isEnabled =
            agent.visible_tools === null || agent.visible_tools.includes(tool);
          const isDisabled = nativeSearchActive && tool !== "native_search";
          ...
          return (
            <button
              ...
              } ${isDisabled ? "opacity-30 cursor-not-allowed" : "hover:bg-accent-primary/10"}`}
            >
              {tool}
            </button>
          );
...
function PromptEditorModal() {
  const editingPromptName = useOrchestratorStore((s) => s.editingPromptName);
  const editingPromptContent = useOrchestratorStore((s) => s.editingPromptContent);
