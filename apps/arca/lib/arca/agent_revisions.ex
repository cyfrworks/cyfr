...
defmodule Arca.AgentRevisions do
  ...
  """
  @spec put(Prima.Actor.t(), binary()) :: {:ok, String.t()} | {:error, term()}
  def put(%Prima.Actor{athanor_id: athanor_id}, bytes)
    ...
    Arca.Repo.Errors.with_db_rescue("Arca.AgentRevisions.put", fn ->
      row = %{
        id: Prima.UUID7.generate_id("agr"),
  ...
  def put(%Prima.Actor{}, _bytes), do: {:error, :no_athanor}
  ...
  def get(%Prima.Actor{athanor_id: athanor_id}, digest)
  ...
  def get(%Prima.Actor{}, _digest), do: {:error, :no_athanor}

  defp verify(bytes, digest) do
    if Prima.Digest.sha256(bytes) == digest, do: {:ok, bytes}, else: {:error, :corrupt}
  end
end
