defmodule Arca.CronSchedule do
  @moduledoc """
  Ecto schema for cron schedule records stored in SQLite.

  Stores user-scoped recurring schedules for WASM component execution.
  """

  use Ecto.Schema
  import Ecto.Changeset
  import Ecto.Query, except: [update: 2]

  @primary_key {:id, :string, autogenerate: false}
  @timestamps_opts []

  schema "cron_schedules" do
    field :user_id, :string
    field :name, :string
    field :cron_expression, :string
    field :reference, :string
    field :input, :string
    field :metadata, :string
    field :status, :string, default: "active"
    field :last_run_at, :utc_datetime_usec
    field :next_run_at, :utc_datetime_usec
    field :last_execution_id, :string
    field :run_count, :integer, default: 0
    field :error_count, :integer, default: 0
    field :created_at, :utc_datetime_usec
    field :updated_at, :utc_datetime_usec
  end

  @doc "Creates a new cron schedule."
  def create(attrs) do
    now = DateTime.utc_now()

    attrs =
      attrs
      |> Map.put_new(:id, generate_id())
      |> Map.put_new(:created_at, now)
      |> Map.put_new(:updated_at, now)

    %__MODULE__{}
    |> cast(Map.new(attrs), [
      :id, :user_id, :name, :cron_expression, :reference,
      :input, :metadata, :status, :next_run_at, :created_at, :updated_at
    ])
    |> validate_required([:id, :user_id, :name, :cron_expression, :reference, :created_at, :updated_at])
    |> validate_inclusion(:status, ["active", "paused", "deleted"])
    |> Arca.Repo.insert()
  end

  @doc "Updates an existing cron schedule."
  def update(id, attrs) do
    case Arca.Repo.get(__MODULE__, id) do
      nil ->
        {:error, :not_found}

      schedule ->
        attrs = Map.put(attrs, :updated_at, DateTime.utc_now())

        schedule
        |> cast(Map.new(attrs), [
          :name, :cron_expression, :reference, :input, :metadata,
          :status, :next_run_at, :last_run_at, :last_execution_id,
          :run_count, :error_count, :updated_at
        ])
        |> Arca.Repo.update()
    end
  end

  @doc "Gets a schedule by ID."
  def get(id) do
    Arca.Repo.get(__MODULE__, id)
  end

  @doc "Gets a schedule by user_id and either ID or name."
  def get_by_user(user_id, id_or_name) do
    from(s in __MODULE__,
      where: s.user_id == ^user_id and s.status != "deleted",
      where: s.id == ^id_or_name or s.name == ^id_or_name
    )
    |> Arca.Repo.one()
  end

  @doc "Lists schedules for a user."
  def list_by_user(user_id, opts \\ []) do
    limit = Keyword.get(opts, :limit, 50)

    from(s in __MODULE__,
      where: s.user_id == ^user_id and s.status != "deleted",
      order_by: [desc: s.created_at],
      limit: ^limit
    )
    |> Arca.Repo.all()
  end

  @doc "Returns all active schedules."
  def active_schedules do
    from(s in __MODULE__,
      where: s.status == "active",
      order_by: [asc: s.next_run_at]
    )
    |> Arca.Repo.all()
  end

  @doc "Returns active schedules that are due (next_run_at <= now)."
  def due_schedules(now \\ DateTime.utc_now()) do
    from(s in __MODULE__,
      where: s.status == "active" and s.next_run_at <= ^now,
      order_by: [asc: s.next_run_at]
    )
    |> Arca.Repo.all()
  end

  @doc "Records a successful run."
  def record_run(id, execution_id) do
    case Arca.Repo.get(__MODULE__, id) do
      nil ->
        {:error, :not_found}

      schedule ->
        schedule
        |> cast(
          %{
            last_run_at: DateTime.utc_now(),
            last_execution_id: execution_id,
            run_count: schedule.run_count + 1,
            updated_at: DateTime.utc_now()
          },
          [:last_run_at, :last_execution_id, :run_count, :updated_at]
        )
        |> Arca.Repo.update()
    end
  end

  @doc "Records an error on a schedule."
  def record_error(id, _reason) do
    case Arca.Repo.get(__MODULE__, id) do
      nil ->
        {:error, :not_found}

      schedule ->
        schedule
        |> cast(
          %{
            error_count: schedule.error_count + 1,
            updated_at: DateTime.utc_now()
          },
          [:error_count, :updated_at]
        )
        |> Arca.Repo.update()
    end
  end

  @doc "Soft-deletes a schedule."
  def soft_delete(id) do
    case Arca.Repo.get(__MODULE__, id) do
      nil -> {:error, :not_found}
      schedule ->
        schedule
        |> cast(%{status: "deleted", updated_at: DateTime.utc_now()}, [:status, :updated_at])
        |> Arca.Repo.update()
    end
  end

  @doc "Counts active schedules for a user."
  def count_by_user(user_id) do
    from(s in __MODULE__,
      where: s.user_id == ^user_id and s.status != "deleted",
      select: count(s.id)
    )
    |> Arca.Repo.one()
  end

  defp generate_id do
    "sched_" <> Ecto.UUID.generate()
  end
end
