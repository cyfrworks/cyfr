...
defmodule Arca.ExecutionStanding do
  @moduledoc """
  The rows an admitted execution's grant (`Cyfr.ExecutionGrant`) is held
  ...
  `Cyfr.ExecutionGrant` or `:stored`, the stamp its attempt row carries —
  ...
  @type verify :: (Cyfr.ExecutionGrant.t() -> :ok | {:error, term()})
  ...
  """
  @spec current(Cyfr.Actor.t(), String.t()) ::
          {:ok, map() | nil} | {:error, :database_error | :cross_tenant}
  def current(%Cyfr.Actor{athanor_id: athanor_id}, athanor_id)
  ...
  def current(%Cyfr.Actor{}, _athanor_id), do: {:error, :cross_tenant}
  ...
  """
  @spec locked(Cyfr.Actor.t(), String.t()) ::
          {:ok, map() | nil} | {:error, :database_error | :cross_tenant}
  def locked(%Cyfr.Actor{athanor_id: athanor_id}, athanor_id)
      when is_binary(athanor_id) and athanor_id != "" do
  ...
  def locked(%Cyfr.Actor{}, _athanor_id), do: {:error, :cross_tenant}
  ...
  def retired_attempts(%Cyfr.Actor{scope: :platform, system: true}, cursor, limit)
  ...
  def retired_attempts(%Cyfr.Actor{}, _cursor, _limit), do: {:error, :cross_tenant}
  ...
  @spec inputs(keyword() | map(), (-> Cyfr.ExecutionGrant.t() | nil)) ::
          {:ok, Cyfr.ExecutionGrant.t() | nil, verify()} | {:error, :missing_grant}
  def inputs(opts, stored) when is_function(stored, 0) do
    ...
    case {get.(:grant), get.(:verify)} do
      ...
      {%Cyfr.ExecutionGrant{} = grant, verify} when is_function(verify, 1) ->
        if Cyfr.ExecutionGrant.valid?(grant),
          do: {:ok, grant, verify},
  ...
  @doc false
  @spec verify!(Cyfr.ExecutionGrant.t(), verify()) :: :ok
  ...
  def verify!(%Cyfr.ExecutionGrant{} = grant, verify) when is_function(verify, 1) do
  ...
  @doc false
  @spec stored(Cyfr.Actor.t() | :any, String.t()) :: Cyfr.ExecutionGrant.t() | nil
  ...
  @doc false
  @spec stored_of_execution(Cyfr.Actor.t(), String.t()) :: Cyfr.ExecutionGrant.t() | nil
