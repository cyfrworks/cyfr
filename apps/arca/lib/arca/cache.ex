...
defmodule Arca.Cache do
  ...
  @doc """
  Make sure the ETS cache table is up, whoever asks.

  The table is `Arca.Cache.Sweeper`'s, and only the sweeper's: an ETS
  table dies with the process that created it, so a caller that made one
  of its own would take the whole cache with it when it exits. Any other
  caller is therefore routed to the sweeper
  (`Arca.Cache.Sweeper.ensure_table/0`) rather than creating a table it
  ...
  def init do
    cond do
      ...
      self() == Process.whereis(Arca.Cache.Sweeper) ->
        :ets.new(@table_name, [
          :set,
          :public,
          :named_table,
      ...
      true ->
        # Answers `{:error, :cache_unavailable}` when no sweeper is running
        # at all; the cache is a read-through and a caller that cannot have
