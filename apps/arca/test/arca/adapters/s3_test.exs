...
defmodule Arca.Adapters.S3Test do
  ...
  describe "put/3" do
    ...
    test "an athanor named after a reserved root keys under athanors/, not the root", %{
      ...
      odd_actor = %{actor | athanor_id: "components"}
      assert :ok = S3.put(odd_actor, ["data", "b.json"], "{}")
    ...
    test "the bare components root is the actor's athanor's subtree", %{actor: actor} do
    ...
    test "applies CYFR_S3_PREFIX when configured", %{actor: actor} do
      Application.put_env(
        :arca,
        :s3,
        Application.get_env(:arca, :s3) |> Keyword.put(:prefix, "tenants/prod")
      )
  ...
  describe "a connection that fails around a conditional write" do
    ...
    defp point_at(port) do
      ...
      Application.put_env(
        :arca,
        :s3,
        Application.get_env(:arca, :s3) |> Keyword.put(:endpoint, "http://127.0.0.1:#{port}")
      )
    end
