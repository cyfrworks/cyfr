defmodule Arca.CronScheduleTest do
  ...
  describe "get_by_user/2" do
    test "finds by name" do
      ...
      found = CronSchedule.get_by_user("test_user", "find-me")
      assert found.id == schedule.id
    end

    test "finds by id" do
      {:ok, schedule} = CronSchedule.create(valid_attrs())
      found = CronSchedule.get_by_user("test_user", schedule.id)
      assert found.id == schedule.id
    end

    test "does not find deleted schedules" do
      {:ok, schedule} = CronSchedule.create(valid_attrs(%{name: "deleted-one"}))
      CronSchedule.soft_delete(schedule.id)
      assert CronSchedule.get_by_user("test_user", "deleted-one") == nil
    end

    test "does not find other user's schedules" do
      {:ok, _} = CronSchedule.create(valid_attrs(%{name: "private", user_id: "other_user"}))
      assert CronSchedule.get_by_user("test_user", "private") == nil
    end
  end

  describe "list_by_user/2" do
    test "lists only non-deleted schedules for user" do
      ...
      CronSchedule.soft_delete(s2.id)

      schedules = CronSchedule.list_by_user("test_user")
  ...
  describe "record_run/2" do
    test "increments run_count and sets last_run_at" do
      {:ok, schedule} = CronSchedule.create(valid_attrs())
      assert {:ok, updated} = CronSchedule.record_run(schedule.id, "exec_123")
  ...
  describe "record_error/2" do
    test "increments error_count" do
      {:ok, schedule} = CronSchedule.create(valid_attrs())
      assert {:ok, updated} = CronSchedule.record_error(schedule.id, "boom")
      assert updated.error_count == 1
    end
  end

  describe "count_by_user/1" do
    test "counts non-deleted schedules" do
      {:ok, _} = CronSchedule.create(valid_attrs(%{name: "c1"}))
      {:ok, s2} = CronSchedule.create(valid_attrs(%{name: "c2"}))
      CronSchedule.soft_delete(s2.id)

      assert CronSchedule.count_by_user("test_user") == 1
    end
  end
end
