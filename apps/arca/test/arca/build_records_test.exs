...
defmodule Arca.BuildRecordsTest do
  ...
  describe "the actor is the only tenant this facade accepts" do
    setup do
      # A plain map carrying the fields an actor does — what a caller that
      # assembled its own tenancy by hand would hand over. The heads match
      # the struct, so it reaches no clause.
      not_actor = %{
        user_id: "usr_ctx",
        athanor_id: "ath_ctx",
        scope: :athanor,
      ...
      {:ok, not_actor: not_actor}
    end

    @tag :no_connection
    test "a plain map of an actor's fields matches no head", %{not_actor: not_actor} do
      assert_raise FunctionClauseError, fn ->
        BuildRecords.get(not_actor, "build_ctx")
      end

      assert_raise FunctionClauseError, fn ->
        BuildRecords.record_started(not_actor, "build_ctx", "reagent:local.demo:0.1.0")
      end

      assert_raise FunctionClauseError, fn ->
        BuildRecords.record_finished(not_actor, "build_ctx", "failed", "no")
      end

      assert_raise FunctionClauseError, fn ->
        BuildRecords.prune(not_actor, 10)
      end
    end
    ...
    # an unresolved tenant looks like once it has been through a string: the
    # identity domain refuses it when it resolves a caller and
    # `Arca.QueryHelpers.where_tenant/2` raises on it, so a facade that merely
