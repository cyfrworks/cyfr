...
defmodule Arca.StorageTest do
  ...
  describe "tenant_segments/1" do
    ...
    test "raises for a platform actor with no athanor too" do
      actor = Arca.Test.Actor.platform(user_id: "system")
  ...
  describe "Arca.exists?/2 is a total predicate" do
    test "a traversal segment under a legal root answers false, never raises" do
      ...
      refute Arca.exists?(actor, ["data", "..", "aqua"])
      refute Arca.exists?(actor, ["data", ".."])
      refute Arca.exists?(actor, ["nope", "x"])
      refute Arca.exists?(actor, ["data", String.duplicate("a", 500)])

      # Every other facade entry keeps failing loud on the same input.
      assert_raise ArgumentError, fn ->
        Arca.get(actor, ["data", "..", "aqua"])
      end
    end

    test "an athanor-less actor answers false for a tenant path, never raises" do
      actor = Arca.Test.Actor.platform()
  ...
  describe "mutating ops refuse root and scope-root paths" do
    ...
    test "put/append/delete below depth 2 answer {:error, :invalid_path}" do
      ...
      assert {:error, :invalid_path} = Arca.put(actor, [], "x")
      assert {:error, :invalid_path} = Arca.put(actor, ["data"], "x")
      assert {:error, :invalid_path} = Arca.append(actor, ["data"], "x")
    ...
    test "a multi-level string segment counts as its real depth" do
      ...
      assert :ok = Arca.put(actor, ["data/depth_gate_pin.txt"], "x")
      assert :ok = Arca.delete(actor, ["data/depth_gate_pin.txt"])
    end

    test "delete_tree keeps working on the whole tree and on a scope" do
      actor = Arca.Test.Actor.local()

      assert :ok = Arca.put(actor, ["data", "depth_gate_tree", "a.txt"], "x")
      assert :ok = Arca.delete_tree(actor, ["data", "depth_gate_tree"])
    end
  end
