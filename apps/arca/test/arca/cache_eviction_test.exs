...
defmodule Arca.CacheEvictionTest do
  ...
  test "binary values are bounded in bytes, oldest-expiring evicted first" do
    original = Application.get_env(:arca, :cache_max_binary_bytes)
    Application.put_env(:arca, :cache_max_binary_bytes, 1_000)

    on_exit(fn ->
      if original,
        do: Application.put_env(:arca, :cache_max_binary_bytes, original),
        else: Application.delete_env(:arca, :cache_max_binary_bytes)
  ...
  test "compiled components get their own entry cap — byte_size cannot see NIF memory" do
    original = Application.get_env(:arca, :cache_max_compiled_components)
    Application.put_env(:arca, :cache_max_compiled_components, 2)

    on_exit(fn ->
      if original,
        do: Application.put_env(:arca, :cache_max_compiled_components, original),
        else: Application.delete_env(:arca, :cache_max_compiled_components)
